...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/.cowagent`) — including agent settings, user identity, and memory files.
...
- **Custom skills:** Created by users through conversation, stored in the workspace (`~/.cowagent/skills/`), capable of implementing any complex business process or third-party integration.
