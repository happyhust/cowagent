...
<Note>
  此步骤为推荐操作。安装后可以使用 `cow start`、`cow stop`、`cow update` 等命令管理服务，也可以使用 `cow skill` 管理技能。如果不安装 CLI，可以使用 `./run.sh` 或 `python3 -m cowagent` 运行。
</Note>
...
| `agent_workspace` | - | Agent 工作空间路径 | `~/.cowagent` |
