...
<Note>
  このステップは推奨です。インストール後、`cow start`、`cow stop`、`cow update` でサービスを管理でき、`cow skill` でスキルを管理できます。CLI をインストールしない場合は、`./run.sh` または `python3 -m cowagent` で実行できます。
</Note>
...
| `agent_workspace` | Agent のワークスペースパス | `~/.cowagent` |
