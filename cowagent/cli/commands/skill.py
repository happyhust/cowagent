...
def _download_repo_zip(
    spec: str, branch: str = "main", host: str = "github", timeout: int = 30
):
...
def _download_github_dir(owner, repo, branch, subpath, dest_dir):
    ...
    resp = requests.get(
        api_url, timeout=30, headers={"Accept": "application/vnd.github.v3+json"}
    )
    resp.raise_for_status()
    items = resp.json()
...
def _scan_skills_in_repo(repo_root: str) -> list:
    ...
    if os.path.isfile(os.path.join(repo_root, "SKILL.md")):
        fm = _parse_skill_frontmatter(
            _read_file_text(os.path.join(repo_root, "SKILL.md"))
        )
...
def _batch_install_skills(
    discovered, spec, skills_dir, source, result: InstallResult, display_name: str = ""
):
    ...
    for sname, sdir in discovered:
        ...
        _register_installed_skill(
            safe_name, source=source, display_name=display_name if single else ""
        )
    ...
    if result.installed:
        result.messages.append(
            f"{len(result.installed)} skill(s) installed from {spec}."
        )
    else:
...
def _register_installed_skill(
    name: str, source: str = "cowhub", display_name: str = ""
):
...
def _install_url(url: str, result: InstallResult):
    ...
    if os.path.isdir(skill_dir):
        result.messages.append(
            f"Skill '{skill_name}' already exists. Overwriting SKILL.md ..."
        )
...
def _install_archive_url(url: str, result: InstallResult):
    ...
    if lower_url.endswith((".tar.gz", ".tgz")) or "gzip" in content_type:
        ...
        _install_zip_bytes(
            resp.content, fallback_name, skills_dir, result=result, source_label="url"
        )


def _install_targz_bytes(
    content: bytes, name: str, skills_dir: str, result: InstallResult
):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for member in tf.getmembers():
                ...
                if not resolved.startswith(os.path.realpath(extract_dir)):
                    raise SkillInstallError(
                        "Archive contains path traversal, aborting."
                    )
        ...
        if len(top_items) == 1 and os.path.isdir(
            os.path.join(extract_dir, top_items[0])
        ):
...
def _validate_github_spec(spec: str):
    ...
    if not re.match(r"^[a-zA-Z0-9_\-]+/[a-zA-Z0-9_.\-]+$", spec):
        click.echo(
            f"Error: Invalid GitHub spec '{spec}'. Expected format: owner/repo",
            err=True,
        )
        sys.exit(1)
...
def _check_github_spec(spec: str):
    ...
    if not re.match(r"^[a-zA-Z0-9_\-]+/[a-zA-Z0-9_.\-]+$", spec):
        raise SkillInstallError(
            f"Invalid GitHub spec '{spec}'. Expected format: owner/repo"
        )
...
def _is_junk_entry(filename: str) -> bool:
    ...
    return any(
        p in _JUNK_NAMES or p == "__MACOSX" or p.startswith("._.") for p in parts
    )
...
def search(query):
    ...
    try:
        resp = requests.get(
            f"{SKILL_HUB_API}/skills/search", params={"q": query}, timeout=10
        )
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
...
def _route_install(name: str, result: InstallResult):
    ...
    # --- Direct SKILL.md URL ---
    if name.startswith(("http://", "https://")) and name.rstrip("/").endswith(
        "SKILL.md"
    ):
    ...
    if name.startswith(("http://", "https://")) and re.search(
        r"\.(zip|tar\.gz|tgz)(\?.*)?$", name, re.IGNORECASE
    ):
...
def _install_hub(name, result: InstallResult, provider=None):
    ...
    if "application/json" in content_type:
        ...
        if source_type == "github":
            ...
            try:
                ...
                if parsed_url:
                    ...
                    _install_github(
                        source_url, result, skill_name=name, timeout=gh_timeout
                    )
                if hub_display_name:
            ...
            result.messages.append(
                f"GitHub download failed ({gh_err}), trying mirror..."
            )
            try:
            ...
            if len(result.installed) == installed_before:
                _register_installed_skill(
                    name, source="cowhub", display_name=hub_display_name
                )
                result.installed.append(name)
        ...
        if source_type == "registry":
            ...
            if download_url:
                ...
                try:
                    ...
                    if not has_mirror:
                        raise SkillInstallError(
                            f"Failed to download from {src_provider}: {e}"
                        )
                ...
                if dl_err is None:
                    ...
                    _install_zip_bytes(
                        ...
                        skills_dir,
                        result=result,
                        source_label=src_provider,
                        display_name=hub_display_name,
                    )
                    if len(result.installed) == installed_before:
                        _register_installed_skill(
                            name, source=src_provider, display_name=hub_display_name
                        )
                        result.installed.append(name)
                        result.messages.append(
                            f"Installed '{name}' from {src_provider}."
                        )
                    return

                # Fallback: download mirror from Skill Hub
                result.messages.append(
                    f"Direct download failed ({dl_err}), trying mirror..."
                )
                try:
                    mirror_resp = requests.post(
                        f"{SKILL_HUB_API}/skills/{name}/download",
        ...
        installed_before = len(result.installed)
        _install_zip_bytes(
            resp.content, name, skills_dir, result=result, source_label="cowhub"
        )
        if len(result.installed) == installed_before:
    ...
    if repo_root:
        try:
            _install_from_repo_root(
                repo_root, spec, subpath, skill_name, skills_dir, source, result
            )
            return
        except SkillInstallError:
            raise
...
def _install_from_repo_root(
    repo_root, spec, subpath, skill_name, skills_dir, source, result: InstallResult
):
    ...
    if subpath:
        ...
        raise SkillInstallError(
            f"No SKILL.md found in '{subpath}' or its subdirectories."
        )
    else:
...
def _install_gitlab(spec, result: InstallResult, subpath=None, branch="main"):
    ...
    try:
        _install_from_repo_root(
            repo_root, spec, subpath, None, skills_dir, "gitlab", result
        )
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def _install_git_clone(git_url: str, result: InstallResult, display_name: str = ""):
    ...
    try:
        _install_from_repo_root(
            repo_root, display_name or git_url, None, None, skills_dir, "git", result
        )
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def _install_zip_bytes(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if discovered and len(discovered) == 1:
            ...
            shutil.copytree(sdir, target)
            _register_installed_skill(
                safe_name, source=source_label, display_name=display_name
            )
