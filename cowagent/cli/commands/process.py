...
from cowagent.cli.utils import get_project_root

_IS_WIN = sys.platform == "win32"


def _get_pid_file():
    return os.path.join(get_project_root(), "cowagent.pid")


def _get_log_file():
    return os.path.join(get_project_root(), "logs", "cowagent.log")
...
def start(foreground, no_logs):
    ...
    if foreground:
        ...
        if _IS_WIN:
            sys.exit(subprocess.call([python, "-m", "cowagent"], cwd=root))
        else:
            os.execv(python, [python, "-m", "cowagent"])
    else:
...
def update(ctx):
    ...
    if _IS_WIN:
        ...
        lines += [
            ...
            f'"{python}" -m cowagent.cli.cli start --no-logs',
            "echo.",
...
def status():
    ...
    from cowagent.cli.utils import load_config_json
