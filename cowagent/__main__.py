...
class ChannelManager:
        ...
        with self._lock:
            ...
            if first_start:
                ...
                if conf().get("use_linkai") and (
                    ...
                    try:
                        from cowagent.common import cloud_client
            ...
            for i, (name, ch) in enumerate(ordered):
                if i > 0 and name != "web":
                    time.sleep(0.1)
                t = threading.Thread(
                    target=self._run_channel, args=(name, ch), daemon=True
                )
    ...
    def stop(self, channel_name: str = None):
        ...
        with self._lock:
            ...
            if channel_name and self._primary_channel is self._channels.get(
                channel_name
            ):
                self._primary_channel = None
        ...
        for name, ch, th in to_stop:
            if ch is None:
                logger.warning(
                    f"[ChannelManager] Channel '{name}' not found in managed channels"
                )
            ...
            if hasattr(ch, "stop"):
                try:
                    ...
                    logger.warning(
                        f"[ChannelManager] Error during channel '{name}' stop: {e}"
                    )
            if th and th.is_alive():
                th.join(timeout=5)
                if th.is_alive():
                    if graceful:
                        ...
                        logger.warning(
                            f"[ChannelManager] Channel '{name}' thread did not exit in 5s, forcing interrupt"
                        )
                        self._interrupt_thread(th, name)
    ...
    def _interrupt_thread(th: threading.Thread, name: str):
        ...
        try:
            ...
            if res == 1:
                ...
                logger.warning(
                    f"[ChannelManager] Failed to interrupt thread for channel '{name}'"
                )
        except Exception as e:
    ...
    def restart(self, new_channel_name: str):
        ...
        logger.info(
            f"[ChannelManager] Channel restarted to '{new_channel_name}' successfully"
        )
    ...
    def add_channel(self, channel_name: str):
        ...
        with self._lock:
            if channel_name in self._channels:
                logger.info(
                    f"[ChannelManager] Channel '{channel_name}' already exists, restarting"
                )
    ...
    def remove_channel(self, channel_name: str):
        ...
        with self._lock:
            if channel_name not in self._channels:
                logger.warning(
                    f"[ChannelManager] Channel '{channel_name}' not found, nothing to remove"
                )
                return
...
def _clear_singleton_cache(channel_name: str):
    ...
    """
    cls_map = {
        "web": "cowagent.channel.web.web_channel.WebChannel",
        "wechatmp": "cowagent.channel.wechatmp.wechatmp_channel.WechatMPChannel",
        "wechatmp_service": "cowagent.channel.wechatmp.wechatmp_channel.WechatMPChannel",
        "wechatcom_app": "cowagent.channel.wechatcom.wechatcomapp_channel.WechatComAppChannel",
        const.FEISHU: "cowagent.channel.feishu.feishu_channel.FeiShuChanel",
        const.DINGTALK: "cowagent.channel.dingtalk.dingtalk_channel.DingTalkChanel",
        const.WECOM_BOT: "cowagent.channel.wecom_bot.wecom_bot_channel.WecomBotChannel",
        const.QQ: "cowagent.channel.qq.qq_channel.QQChannel",
        const.WEIXIN: "cowagent.channel.weixin.weixin_channel.WeixinChannel",
        "wx": "cowagent.channel.weixin.weixin_channel.WeixinChannel",
    }
            ...
            for cell in wrapper.__closure__:
                try:
                    ...
                    if isinstance(cell_contents, dict):
                        cell_contents.clear()
                        logger.debug(
                            f"[ChannelManager] Cleared singleton cache for {class_name}"
                        )
                        break
                except ValueError:
                    pass
