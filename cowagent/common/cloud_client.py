...
class CloudClient(LinkAIClient):
    ...
    def on_message(self, push_msg: PushMsg):
        ...
        logger.info(
            f"receive msg push, session_id={session_id}, msg_content={msg_content}"
        )
        context = Context()
    ...
    def on_config(self, config: dict):
        ...
        if self._set_channel_credentials(
            local_config,
        ...
        if (
            config.get("text_to_image")
            and config.get("text_to_image") == "midjourney"
            and pconf("linkai")
        ):
            if pconf("linkai")["midjourney"]:
                pconf("linkai")["midjourney"]["enabled"] = True
                pconf("linkai")["midjourney"]["use_image_create_prefix"] = True
        elif config.get("text_to_image") and config.get("text_to_image") in [
            "dall-e-2",
            "dall-e-3",
        ]:
            if pconf("linkai")["midjourney"]:
    ...
    def _dispatch_channel_action(self, action: str, data: dict):
        ...
        if not channel_type:
            logger.warning(
                f"[CloudClient] Channel action '{action}' missing channelType, data={data}"
            )
            return
        logger.info(
            f"[CloudClient] Channel action: {action}, channelType={channel_type}"
        )
    ...
    def _handle_channel_create(self, channel_type: str, data: dict):
        ...
        if skip_restart and channel_type in ("weixin", "wx"):
            ...
            if login_status != "logged_in":
                skip_restart = False
                logger.info(
                    f"[CloudClient] Channel '{channel_type}' not logged in "
                    f"(status={login_status}), forcing restart"
                )
        if skip_restart:
            logger.info(
                f"[CloudClient] Channel '{channel_type}' already running with same config, "
    ...
    def _handle_channel_update(self, channel_type: str, data: dict):
        ...
        if enabled == "N":
            ...
            if existing_ch and not needs_restart:
                ...
                threading.Thread(
                    target=self._report_channel_startup,
                    args=(channel_type,),
                    daemon=True,
                ).start()
            else:
                threading.Thread(
                    target=self._do_restart_channel,
                    args=(self.channel_mgr, channel_type),
                    daemon=True,
                ).start()
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _set_channel_credentials(
        local_config: dict, channel_type: str, app_id, app_secret
    ) -> bool:
        """
        ...
        if changed:
            logger.info(
                f"[CloudClient] Synced {channel_type} credentials to conf and env"
            )
        return changed
    ...
    def _do_add_channel(self, channel_type: str):
        try:
            ...
            logger.error(
                f"[CloudClient] Failed to add channel '{channel_type}': {e}",
                exc_info=True,
            )
    ...
    def _do_remove_channel(self, channel_type: str):
        try:
            ...
            logger.error(
                f"[CloudClient] Failed to remove channel '{channel_type}': {e}"
            )
    ...
    def _report_channel_startup(self, channel_type: str):
        ...
        if not ch:
            self.send_channel_status(
                channel_type, "error", "channel instance not found"
            )
            return
        ...
        if channel_type in ("weixin", "wx") and hasattr(ch, "login_status"):
            ...
            if login_status in ("waiting_scan", "scanned", "idle"):
                logger.info(
                    f"[CloudClient] Channel '{channel_type}' is waiting for QR login, "
        ...
        if success:
            logger.info(
                f"[CloudClient] Channel '{channel_type}' connected, reporting status"
            )
            self.send_channel_status(channel_type, "connected")
        else:
            logger.warning(
                f"[CloudClient] Channel '{channel_type}' startup failed: {error}"
            )
    ...
    def on_skill(self, data: dict) -> dict:
        ...
        if svc is None:
            return {
                "action": action,
                "code": 500,
                "message": "SkillService not available",
                "payload": None,
            }
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        logger.info(
            f"[CloudClient] on_chat: session={session_id}, channel={channel_type}, query={query[:80]}"
        )
        ...
        try:
            ...
            if instance and hasattr(instance, "execute"):
                ...
                if result is not None:
                    send_chunk_fn(
                        {"chunk_type": "content", "delta": result, "segment_id": 0}
                    )
                    return
        except Exception as e:
    ...
    def _query_history(self, payload: dict) -> dict:
        ...
        if not session_id.startswith("session_"):
            session_id = f"session_{session_id}"
        logger.info(
            f"[CloudClient] history query: session={session_id}, page={page}, page_size={page_size}"
        )

        try:
            from cowagent.agent.memory.conversation_store import get_conversation_store
    ...
    def _restart_channel(self, new_channel_type: str):
        ...
        if self.channel_mgr:
            ...
            threading.Thread(
                target=self._do_restart_channel,
                args=(self.channel_mgr, new_channel_type),
                daemon=True,
            ).start()
        else:
            logger.warning(
                "[CloudClient] ChannelManager not available, please restart the application manually"
            )
    ...
    def _do_restart_channel(self, mgr, new_channel_type: str):
        ...
        try:
            ...
            if mgr.channel:
                ...
                logger.info(
                    f"[CloudClient] Channel reference updated to '{new_channel_type}'"
                )
        except Exception as e:
    ...
    def _save_config_to_file(self, local_config: dict):
        ...
        try:
            ...
            if not os.path.exists(config_path):
                logger.warning(
                    f"[CloudClient] config.json not found at {config_path}, skip saving"
                )
                return
            ...
            logger.error(
                f"[CloudClient] Failed to save configuration to config.json: {e}"
            )
...
def get_deployment_id() -> str:
    ...
    return os.environ.get("CLOUD_DEPLOYMENT_ID") or conf().get(
        "cloud_deployment_id", ""
    )
...
def get_website_base_url() -> str:
    ...
    websites_domain = os.environ.get("CLOUD_WEBSITES_DOMAIN") or conf().get(
        "cloud_websites_domain", ""
    )
    if websites_domain:
...
def start(channel, channel_mgr=None):
    ...
    if chat_client.client_id:
        ...
        if channel_mgr:
            ...
            threading.Thread(
                target=_report_existing_channels,
                args=(chat_client, channel_mgr),
                daemon=True,
            ).start()
