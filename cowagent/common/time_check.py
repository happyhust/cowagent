...
def time_checker(f):
    def _time_checker(self, *args, **kwargs):
        ...
        if chat_time_module:
            ...
            if not (
                time_regex.match(chat_start_time) and time_regex.match(chat_stop_time)
            ):
                logger.warning(
                    "时间格式不正确，请在config.json中修改CHAT_START_TIME/CHAT_STOP_TIME。"
                )
                return None
            ...
            # 结束时间小于开始时间，跨天了
            if chat_stop_time < chat_start_time and (
                chat_start_time <= now_time or now_time <= chat_stop_time
            ):
                f(self, *args, **kwargs)
            # 结束大于开始时间代表，没有跨天
            elif (
                chat_start_time < chat_stop_time
                and chat_start_time <= now_time <= chat_stop_time
            ):
                f(self, *args, **kwargs)
            else:
