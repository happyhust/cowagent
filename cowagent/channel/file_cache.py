...
class FileCache:
    ...
    def add(self, session_id: str, file_path: str, file_type: str = "image"):
        ...
        if session_id not in self.cache:
            self.cache[session_id] = {"files": [], "timestamp": time.time()}
        ...
        if file_info not in self.cache[session_id]["files"]:
            ...
            logger.info(
                f"[FileCache] Added {file_type} to cache for session {session_id}: {file_path}"
            )
    ...
    def get(self, session_id: str) -> list:
        ...
        if time.time() - item["timestamp"] > self.ttl:
            logger.info(
                f"[FileCache] Cache expired for session {session_id}, clearing..."
            )
    ...
    def cleanup_expired(self):
        ...
        for session_id in expired_sessions:
            del self.cache[session_id]
            logger.debug(
                f"[FileCache] Cleaned up expired cache for session {session_id}"
            )

        if expired_sessions:
            logger.info(
                f"[FileCache] Cleaned up {len(expired_sessions)} expired cache(s)"
            )
