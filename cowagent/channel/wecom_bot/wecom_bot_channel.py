...
from cowagent.channel.wecom_bot.wecom_bot_message import WecomBotMessage
...
class WecomBotChannel(ChatChannel):
    ...
    def _start_ws(self):
        ...
        def _on_message(ws, raw):
            try:
                ...
                logger.error(
                    f"[WecomBot] Failed to handle ws message: {e}", exc_info=True
                )
        ...
        def _on_close(ws, close_status_code, close_msg):
            logger.warning(
                f"[WecomBot] WebSocket closed: status={close_status_code}, msg={close_msg}"
            )
            self._connected = False
        ...
        if errcode is not None and cmd == "":
            if not self._connected:
                if errcode == 0:
                    ...
                    logger.error(
                        f"[WecomBot] Subscribe failed: errcode={errcode}, errmsg={errmsg}"
                    )
    ...
    def _handle_msg_callback(self, data: dict):
        ...
        if wecom_msg.ctype == ContextType.FILE:
            ...
            logger.info(
                f"[WecomBot] File cached for session {session_id}: {wecom_msg.content}"
            )
            return
    ...
    def _handle_event_callback(self, data: dict):
        ...
        if event_type == "enter_chat":
            logger.info(
                f"[WecomBot] User entered chat: {body.get('from', {}).get('userid')}"
            )
        elif event_type == "disconnected_event":
            logger.warning(
                "[WecomBot] Received disconnected_event, another connection took over"
            )
        else:
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if reply.type == ReplyType.TEXT:
            ...
            self._send_file(
                reply.content, receiver, is_group, req_id, media_type="video"
            )
        else:
            logger.warning(
                f"[WecomBot] Unsupported reply type: {reply.type}, falling back to text"
            )
    ...
    def _send_text(
        self, content: str, receiver: str, is_group: bool, req_id: str = None
    ):
    ...
    def _send_image(
        self, img_path_or_url: str, receiver: str, is_group: bool, req_id: str = None
    ):
        ...
        if local_path.startswith(("http://", "https://")):
            try:
                ...
                logger.info(
                    f"[WecomBot] Image downloaded: size={len(resp.content)}, "
        ...
        if not local_path:
            self._send_text(
                "[Image format conversion failed]", receiver, is_group, req_id
            )
            return
        ...
        logger.info(
            f"[WecomBot] Uploading image: path={local_path}, size={file_size} bytes"
        )
    ...
    def _ensure_image_format(file_path: str) -> str:
        ...
        try:
            ...
            if fmt in ("JPEG", "PNG"):
                ...
                logger.info(
                    f"[WecomBot] Image renamed: {file_path} -> {out_path} ({fmt})"
                )
                return out_path
        ...
        try:
            ...
            while quality >= 30:
                ...
                if os.path.getsize(out_path) <= max_bytes:
                    logger.info(
                        f"[WecomBot] Image compressed: quality={quality}, "
            ...
            if os.path.getsize(out_path) <= max_bytes:
                logger.info(
                    f"[WecomBot] Image compressed with resize: {new_size}, "
                    f"size={os.path.getsize(out_path)} bytes"
                )
                return out_path
