...
class ChatChannel(Channel):
        ...
        if first_in:  # context首次传入时，receiver是None，根据类型设置receiver
            ...
            if context.get("isgroup", False):
                ...
                if any(
                    ...
                    logger.debug(
                        f"No need reply, groupName not in whitelist, group_name={group_name}"
                    )
                    return None
            ...
            if context.get("isgroup", False):  # 群聊
                ...
                if context["msg"].to_user_id != context["msg"].actual_user_id:
                    ...
                    if context["msg"].is_at:
                        ...
                        if nick_name and nick_name in nick_name_black_list:
                            # 黑名单过滤
                            logger.warning(
                                f"[chat_channel] Nickname {nick_name} in In BlackList, ignore"
                            )
                            return None
                ...
                if not flag:
                    if context["origin_ctype"] == ContextType.VOICE:
                        logger.info(
                            "[chat_channel]receive group voice, but checkprefix didn't match"
                        )
                    return None
            else:  # 单聊
                ...
                if (
                    match_prefix is not None
                ):  # 判断如果匹配到自定义前缀，则返回过滤掉前缀+空格后的内容
                    content = content.replace(match_prefix, "", 1).strip()
                elif (
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            logger.debug(
                "[chat_channel] type={}, content={}".format(
            ...
            if (
                ...
                try:
                    ...
                    Exception
                ) as e:  # 转换失败，直接使用mp3，对于某些api，mp3也可以识别
                    logger.warning(
                        "[chat_channel]any to wav error, use raw path. " + str(e)
                    )
                    wav_path = file_path
                # 语音识别
                ...
                context.type == ContextType.FUNCTION or context.type == ContextType.FILE
            ):  # 文件消息及函数调用等，当前无默认逻辑
                pass
            else:
                logger.warning(
                    "[chat_channel] unknown context type: {}".format(context.type)
                )
                return
        return reply
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                if reply.type in self.NOT_SUPPORT_REPLYTYPE:
                    logger.error(
                        "[chat_channel]reply type not support: " + str(reply.type)
                    )
                ...
                if reply.type == ReplyType.TEXT:
                    ...
                    if context.get("isgroup", False):
                        ...
                        reply_text = (
                            conf().get("group_chat_reply_prefix", "")
                            + reply_text
                            + conf().get("group_chat_reply_suffix", "")
                        )
                    else:
                        reply_text = (
                            conf().get("single_chat_reply_prefix", "")
                            + reply_text
                            + conf().get("single_chat_reply_suffix", "")
                        )
                    ...
                    logger.error(
                        "[chat_channel] unknown reply type: {}".format(reply.type)
                    )
                    return
            if (
                desire_rtype
                ...
                logger.warning(
                    "[chat_channel] desire_rtype: {}, but reply type: {}".format(
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                # 如果是图片回复但带有文本内容，先发文本再发图片
                    ...
                    reply.type == ReplyType.IMAGE_URL
                    and hasattr(reply, "text_content")
                    and reply.text_content
                ):
                    # 先发送文本
    ...
    def _extract_and_send_images(self, reply: Reply, context: Context):
        ...
        if media_items:
            logger.info(
                f"[chat_channel] Extracted {len(media_items)} media item(s) from reply"
            )

            # Send text first (the frontend will embed video players via renderMarkdown).
            logger.info(
                f"[chat_channel] Sending text content before media: {reply.content[:100]}..."
            )
            self._send(reply, context)
            logger.info(
                f"[chat_channel] Text sent, now sending {len(media_items)} media item(s)"
            )
            ...
            for i, (url, media_type) in enumerate(media_items):
                try:
                    ...
                    if url.startswith(("http://", "https://")):
                        ...
                        logger.warning(
                            f"[chat_channel] Media file not found or invalid URL: {url}"
                        )
                        continue
                    ...
                    logger.info(
                        f"[chat_channel] Sent {media_type} {i + 1}/{len(media_items)}: {url[:50]}..."
                    )

                except Exception as e:
                    logger.error(
                        f"[chat_channel] Failed to send {media_type} {url}: {e}"
                    )
        else:
            # 没有媒体文件，正常发送文本
            self._send(reply, context)
                ...
                if semaphore.acquire(blocking=False):  # 等线程处理完毕才能删除
                    if not context_queue.empty():
                        ...
                        with self.lock:
                            ...
                            assert len(self.futures[session_id]) == 0, (
                                "thread pool error"
                            )
