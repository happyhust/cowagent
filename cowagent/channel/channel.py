...
class Channel(object):
    ...
    def __init__(self):
        ...
        self.cloud_mode = (
            False  # set to True by ChannelManager when running with cloud client
        )
        ...
        if use_agent:
            try:
                ...
                return Bridge().fetch_agent_reply(
                    query=query, context=context, on_event=on_event, clear_history=False
                )
            except Exception as e:
                logger.error(
                    f"[Channel] Agent mode failed, fallback to normal mode: {e}"
                )
