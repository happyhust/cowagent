...
from cowagent.channel.weixin.weixin_api import (
    WeixinApi,
    upload_media_to_cdn,
    DEFAULT_BASE_URL,
    CDN_BASE_URL,
)
from cowagent.channel.weixin.weixin_message import WeixinMessage
...
class WeixinChannel(ChatChannel):
    ...
    def startup(self):
        ...
        logger.info(
            f"[Weixin] 微信通道已启动，凭证保存在 {self._credentials_path}，"
            f"如需重新扫码登录请删除该文件后重启"
        )
    ...
    def _print_qr(qrcode_url: str):
        ...
        try:
            ...
            qr = qr_lib.QRCode(
                error_correction=qr_lib.constants.ERROR_CORRECT_L, box_size=1, border=1
            )
    ...
    def _qr_login(self, base_url: str) -> dict:
        ...
        while not self._stop_event.is_set():
            if time.time() >= deadline:
                logger.warning(
                    f"[Weixin] QR login timed out after {QR_LOGIN_TIMEOUT_S}s"
                )
            ...
            if status == "wait":
                ...
                if refresh_count >= QR_MAX_REFRESHES:
                    logger.warning(
                        f"[Weixin] QR code refreshed {QR_MAX_REFRESHES} times, giving up"
                    )
                    print(
                    ...
                    break
                print(
                    f"  二维码已过期，正在刷新（{refresh_count}/{QR_MAX_REFRESHES}）..."
                )
                try:
                    qr_resp = api.fetch_qr_code()
                    qrcode = qr_resp.get("qrcode", "")
                    ...
                    self._current_qr_url = qrcode_url
                    logger.info(
                        f"[Weixin] 微信二维码链接 ({refresh_count}/{QR_MAX_REFRESHES}): {qrcode_url}"
                    )
                    self._print_qr(qrcode_url)
                    self._notify_cloud_qrcode(qrcode_url)
                except Exception as e:
            ...
            try:
                ...
                if is_error:
                    if (
                        ...
                        logger.error(
                            "[Weixin] Session expired (errcode -14), starting re-login..."
                        )
                        if self._relogin():
                            logger.info(
                                "[Weixin] Re-login successful, resuming long-poll"
                            )
                            self._get_updates_buf = ""
                            consecutive_failures = 0
                            continue
                            ...
                            logger.error(
                                "[Weixin] Re-login failed, will retry in 5 minutes"
                            )
                    ...
                    logger.error(
                        f"[Weixin] getUpdates error: ret={ret} errcode={errcode} "
                ...
                for raw_msg in msgs:
                    try:
                        ...
                        logger.error(
                            f"[Weixin] Failed to process message: {e}", exc_info=True
                        )
                ...
                consecutive_failures += 1
                logger.error(
                    f"[Weixin] getUpdates exception: {e} "
    ...
    def _process_message(self, raw_msg: dict):
        ...
        logger.info(
            f"[Weixin] Received: from={from_user} ctype={wx_msg.ctype} "
        ...
        if wx_msg.ctype == ContextType.FILE:
            ...
            logger.info(
                f"[Weixin] File cached for session {session_id}: {wx_msg.content}"
            )
            return
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if reply.type == ReplyType.TEXT:
            ...
            logger.warning(
                f"[Weixin] Unsupported reply type: {reply.type}, fallback to text"
            )
    ...
    def _send_text(self, text: str, receiver: str, context_token: str):
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                logger.debug(
                    f"[Weixin] Text chunk {i + 1}/{len(chunks)} sent to {receiver}, len={len(chunk)}"
                )
            except Exception as e:
                logger.error(
                    f"[Weixin] Failed to send text chunk {i + 1}/{len(chunks)}: {e}"
                )
                break
            if i < len(chunks) - 1:
                time.sleep(0.5)
