...
class WeixinApi:
    ...
    def _post(
        self, endpoint: str, body: dict, timeout: int = DEFAULT_API_TIMEOUT
    ) -> dict:
    ...
    def get_updates(
        self, get_updates_buf: str = "", timeout: int = DEFAULT_LONG_POLL_TIMEOUT
    ) -> dict:
        return self._post(
...
def upload_media_to_cdn(
    api: WeixinApi, file_path: str, to_user_id: str, media_type: int
) -> dict:
    """
    ...
    for attempt in range(1, UPLOAD_MAX_RETRIES + 1):
        try:
            ...
            if upload_full_url:
                ...
                raise RuntimeError(
                    f"[Weixin] getUploadUrl returned neither upload_full_url nor upload_param: {resp}"
                )
            ...
            if 400 <= cdn_resp.status_code < 500:
                ...
                raise RuntimeError(
                    f"CDN client error {cdn_resp.status_code}: {err_msg}"
                )
            ...
            logger.debug(
                f"[Weixin] CDN upload success attempt={attempt} filekey={filekey}"
            )
            break
        except Exception as e:
            last_error = e
            ...
            if attempt < UPLOAD_MAX_RETRIES:
                backoff = 2**attempt
                logger.warning(
                    f"[Weixin] CDN upload attempt {attempt} failed, retrying in {backoff}s: {e}"
                )
                time.sleep(backoff)
            else:
                logger.error(
                    f"[Weixin] CDN upload failed after {UPLOAD_MAX_RETRIES} attempts: {e}"
                )
...
def download_media_from_cdn(
    cdn_base_url: str, encrypt_query_param: str, aes_key: str, save_path: str
) -> str:
    """
    ...
    try:
        ...
        if len(decoded) == 32:
            ...
            raise ValueError(
                f"Invalid AES key length after base64 decode: {len(decoded)}"
            )
