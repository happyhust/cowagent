        ...
        try:
            ...
            if request_id in self.sse_queues:
                ...
                if reply.type in (
                    ...
                    logger.debug(
                        f"SSE skipped http media reply for request {request_id}"
                    )
                    return
            ...
            if session_id in self.session_queues:
                ...
                logger.debug(
                    f"Response sent to poll queue for session {session_id}, request {request_id}"
                )
            else:
                logger.warning(
                    f"No response queue found for session {session_id}, response dropped"
                )
    ...
    def upload_file(self):
        ...
        try:
            ...
            with open(save_path, "wb") as f:
                f.write(
                    file_obj.read() if hasattr(file_obj, "read") else file_obj.value
                )
            ...
            logger.info(
                f"[WebChannel] File uploaded: {original_name} -> {save_path} ({file_type})"
            )
        ...
        try:
            ...
            if attachments:
                ...
                if file_refs:
                    prompt = prompt + "\n" + "\n".join(file_refs)
                    logger.info(
                        f"[WebChannel] Attached {len(file_refs)} file(s) to message"
                    )
            ...
            if context is None:
                logger.warning(
                    f"[WebChannel] Context is None for session {session_id}, message may be filtered"
                )
                if request_id in self.sse_queues:
                    del self.sse_queues[request_id]
                return json.dumps(
                    {"status": "error", "message": "Message was filtered"}
                )
            ...
            return json.dumps(
                {"status": "success", "request_id": request_id, "stream": use_sse}
            )
...
class ConfigHandler:
            ...
            return json.dumps(
                {
                    ...
                    "agent_max_context_tokens": local_config.get(
    ...
    def POST(self):
        ...
        try:
            ...
            if not applied:
                return json.dumps(
                    {"status": "error", "message": "no valid keys to update"}
                )
            ...
            return json.dumps(
                {"status": "success", "applied": applied}, ensure_ascii=False
            )
        except Exception as e:
...
class ChannelsHandler:
        ...
        [
            ...
            (
                "wechatcom_app",
                {
                    ...
                    "fields": [
                        {
                            "key": "wechatcom_corp_id",
                            "label": "Corp ID",
                            "type": "text",
                        },
                        {
                            "key": "wechatcomapp_agent_id",
                            "label": "Agent ID",
                            "type": "text",
                        },
                        {
                            "key": "wechatcomapp_secret",
                            "label": "Secret",
                            "type": "secret",
                        },
                        {
                            "key": "wechatcomapp_token",
                            "label": "Token",
                            "type": "secret",
                        },
                        {
                            "key": "wechatcomapp_aes_key",
                            "label": "AES Key",
                            "type": "secret",
                        },
                        {
            ...
            (
                "wechatmp",
                {
                    ...
                    "fields": [
                        ...
                        {
                            "key": "wechatmp_app_secret",
                            "label": "App Secret",
                            "type": "secret",
                        },
                        {"key": "wechatmp_token", "label": "Token", "type": "secret"},
                        {
                            "key": "wechatmp_aes_key",
                            "label": "AES Key",
                            "type": "secret",
                        },
                        {
    ...
    def GET(self):
        ...
        try:
            ...
            return json.dumps(
                {"status": "success", "channels": channels}, ensure_ascii=False
            )
        except Exception as e:
    ...
    def POST(self):
        ...
        try:
            ...
            if not action or not channel_name:
                return json.dumps(
                    {"status": "error", "message": "action and channel required"}
                )

            if channel_name not in self.CHANNEL_DEFS:
                return json.dumps(
                    {"status": "error", "message": f"unknown channel: {channel_name}"}
                )
    ...
    def _handle_save(self, channel_name: str, updates: dict):
        ...
        logger.info(
            f"[WebChannel] Channel '{channel_name}' config updated: {list(applied.keys())}"
        )
        ...
        if channel_name in active_channels:
            ...
            try:
                ...
                if mgr:
                    ...
                    logger.info(
                        f"[WebChannel] Channel '{channel_name}' restart triggered"
                    )
            except Exception as e:
                logger.warning(
                    f"[WebChannel] Failed to restart channel '{channel_name}': {e}"
                )
    ...
    def _handle_connect(self, channel_name: str, updates: dict):
        ...
        logger.info(
            f"[WebChannel] Channel '{channel_name}' connecting, channel_type={new_channel_type}"
        )
        ...
        def _do_start():
            try:
                ...
                app_module = sys.modules.get("__main__") or sys.modules.get("app")
                clear_fn = (
                    getattr(app_module, "_clear_singleton_cache", None)
                    if app_module
                    else None
                )
                mgr = getattr(app_module, "_channel_mgr", None) if app_module else None
                if mgr is None:
                    logger.warning(
                        f"[WebChannel] ChannelManager not available, cannot start '{channel_name}'"
                    )
                    return
                ...
                if existing_ch is not None:
                    logger.info(
                        f"[WebChannel] Stopping existing '{channel_name}' before reconnect..."
                    )
                ...
                logger.info(
                    f"[WebChannel] Waiting for '{channel_name}' old connection to close..."
                )
                time.sleep(5)
                if clear_fn:
                ...
                logger.error(
                    f"[WebChannel] Failed to start channel '{channel_name}': {e}",
                    exc_info=True,
                )
        ...
        def _do_stop():
            try:
                ...
                if mgr:
                    ...
                    logger.warning(
                        f"[WebChannel] ChannelManager not found, cannot stop '{channel_name}'"
                    )
                if clear_fn:
                    clear_fn(channel_name)
                logger.info(
                    f"[WebChannel] Channel '{channel_name}' disconnected, "
                    f"channel_type={new_channel_type}"
                )
            except Exception as e:
                logger.warning(
                    f"[WebChannel] Failed to stop channel '{channel_name}': {e}",
                    exc_info=True,
                )
...
class WeixinQrHandler:
    ...
    def _qr_to_data_uri(data: str) -> str:
        ...
        try:
            ...
            qr = qr_lib.QRCode(
                error_correction=qr_lib.constants.ERROR_CORRECT_L, box_size=6, border=2
            )
            qr.add_data(data)
            qr.make(fit=True)
    ...
    def GET(self):
        ...
        try:
            ...
            if (
                running_ch
                and hasattr(running_ch, "_current_qr_url")
                and running_ch._current_qr_url
            ):
            ...
            from cowagent.channel.weixin.weixin_api import WeixinApi, DEFAULT_BASE_URL
            ...
            return json.dumps(
                {"status": "success", "qrcode_url": qrcode_url, "qr_image": qr_image}
            )
        except Exception as e:
    ...
    def _poll_status(self):
        ...
        if qr_status == "confirmed":
            ...
            if not bot_token or not bot_id:
                return json.dumps(
                    {"status": "error", "message": "Login confirmed but missing token"}
                )
...
class SkillsHandler:
    def GET(self):
        ...
        try:
            ...
            return json.dumps(
                {"status": "success", "skills": skills}, ensure_ascii=False
            )
        except Exception as e:
    ...
    def POST(self):
        ...
        try:
            ...
            if not action or not name:
                return json.dumps(
                    {"status": "error", "message": "action and name are required"}
                )
            workspace_root = _get_workspace_root()
            manager = SkillManager(custom_dir=os.path.join(workspace_root, "skills"))
            service = SkillService(manager)
...
class MemoryHandler:
    def GET(self):
        web.header("Content-Type", "application/json; charset=utf-8")
        try:
            from cowagent.agent.memory.service import MemoryService
...
class SchedulerHandler:
    def GET(self):
        web.header("Content-Type", "application/json; charset=utf-8")
        try:
            from cowagent.agent.tools.scheduler.task_store import TaskStore
...
class HistoryHandler:
    def GET(self):
        ...
        try:
            ...
            from cowagent.agent.memory import get_conversation_store
...
class LogsHandler:
    def GET(self):
        ...
        from cowagent.config import get_root
        ...
        def generate():
            ...
            try:
                ...
                payload = json.dumps(
                    {"type": "init", "content": chunk}, ensure_ascii=False
                )
                yield f"data: {payload}\n\n".encode("utf-8")
            except Exception as e:
            ...
            try:
                with open(log_path, "r", encoding="utf-8", errors="replace") as f:
                    ...
                    while time.time() < deadline:
                        ...
                        if line:
                            payload = json.dumps(
                                {"type": "line", "content": line}, ensure_ascii=False
                            )
                            yield f"data: {payload}\n\n".encode("utf-8")
                        else:
                            yield b": keepalive\n\n"
...
class AssetsHandler:
    def GET(self, file_path):  # 修改默认参数
        try:
            ...
            logger.error(
                f"Error serving static file: {e}", exc_info=True
            )  # 添加更详细的错误信息
            raise web.notfound()
...
class VersionHandler:
    def GET(self):
        web.header("Content-Type", "application/json; charset=utf-8")
        from cowagent.cli import __version__
