...
from cowagent.channel.wechatcom.wechatcomapp_client import WechatComAppClient
from cowagent.channel.wechatcom.wechatcomapp_message import WechatComAppMessage
...
class WechatComAppChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        logger.info(
            "[wechatcom] Initializing WeCom app channel, corp_id: {}, agent_id: {}".format(
                self.corp_id, self.agent_id
            )
        )
    ...
    def startup(self):
        ...
        logger.info(
            "[wechatcom] 📡 Listening on http://0.0.0.0:{}/wxcomapp/".format(port)
        )
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if reply.type in [ReplyType.TEXT, ReplyType.ERROR, ReplyType.INFO]:
            ...
            if len(texts) > 1:
                logger.info(
                    "[wechatcom] text too long, split into {} parts".format(len(texts))
                )
            ...
            logger.info(
                "[wechatcom] Do send text to {}: {}".format(receiver, reply_text)
            )
            ...
            try:
                ...
                if len(files) > 1:
                    logger.info(
                        "[wechatcom] voice too long {}s > 60s , split into {} parts".format(
