...
class WechatComAppClient(WeChatClient):
    def __init__(
        ...
        super(WechatComAppClient, self).__init__(
    ...
    def _active_refresh(self):
            ...
            while True:
                ...
                if expires_at - now < 600:
                    with self.fetch_access_token_lock:
                        # 双重检查避免重复刷新
                        if (
                            self.session.get(f"{self.corp_id}_expires_at", 0)
                            - time.time()
                            < 600
                        ):
        ...
        refresh_thread = threading.Thread(
            target=refresh_loop, daemon=True, name="wechatcom_token_refresh_thread"
        )
