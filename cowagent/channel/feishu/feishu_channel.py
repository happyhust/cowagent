...
from cowagent.channel.feishu.feishu_message import FeishuMessage
from cowagent.common import utils
...
class FeiShuChanel(ChatChannel):
    ...
    def __init__(self):
        ...
        logger.debug(
            "[FeiShu] app_id={}, app_secret={}, verification_token={}, event_mode={}".format(
                self.feishu_app_id,
        ...
        if self.feishu_event_mode == "websocket" and not LARK_SDK_AVAILABLE:
            logger.error(
                "[FeiShu] websocket mode requires lark_oapi. Please install: pip install lark-oapi"
            )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("code") == 0:
                    ...
                    logger.warning(
                        f"[FeiShu] Fetch bot info failed: code={data.get('code')}, msg={data.get('msg')}"
                    )
    ...
    def _startup_websocket(self):
        ...
        def handle_message_event(data: lark.im.v1.P2ImMessageReceiveV1) -> None:
            ...
            try:
                ...
                if (
                    msg.get("chat_type") == "group"
                    and not msg.get("mentions")
                    and msg.get("message_type") == "text"
                ):
                    return

                logger.debug(
                    f"[FeiShu] websocket receive event: {lark.JSON.marshal(data, indent=2)}"
                )
                ...
                logger.error(
                    f"[FeiShu] websocket handle message error: {e}", exc_info=True
                )
        ...
        def start_client_with_retry():
            ...
            for attempt in range(2):
                try:
                    if attempt == 1:
                        logger.warning(
                            "[FeiShu] Retrying with SSL verification disabled..."
                        )
    ...
    def _is_mention_bot(self, mentions: list) -> bool:
        ...
        if self._bot_open_id:
            return any(
                m.get("id", {}).get("open_id") == self._bot_open_id for m in mentions
            )
    ...
    def _handle_message_event(self, event: dict):
        ...
        if create_time_ms:
            ...
            if msg_age_s > 60:
                logger.warning(
                    f"[FeiShu] stale msg filtered (age={msg_age_s:.0f}s), msg_id={msg_id}"
                )
                return
        ...
        feishu_msg = FeishuMessage(
            event, is_group=is_group, access_token=self.fetch_access_token()
        )
        if not feishu_msg:
            return
        ...
        if feishu_msg.ctype == ContextType.IMAGE:
            if hasattr(feishu_msg, "image_path") and feishu_msg.image_path:
                ...
                logger.info(
                    f"[FeiShu] Image cached for session {session_id}, waiting for user query..."
                )
            # 单张图片不直接处理，等待用户提问
            return
        ...
        if feishu_msg.ctype == ContextType.TEXT:
            ...
            if cached_files:
                ...
                logger.info(
                    f"[FeiShu] Attached {len(cached_files)} cached file(s) to user query"
                )
                # 清除缓存
    ...
    def send(self, reply: Reply, context: Context):
        ...
        msg_type = "text"
        logger.debug(
            f"[FeiShu] sending reply, type={context.type}, content={reply.content[:100]}..."
        )
        ...
        if reply.type == ReplyType.IMAGE_URL:
            ...
            if hasattr(reply, "text_content") and reply.text_content:
                logger.info(
                    f"[FeiShu] Sending text before file: {reply.text_content[:50]}..."
                )
            ...
            if is_video:
                ...
                msg_type = "media"
                reply_content = (
                    upload_data  # 完整的上传响应数据（包含file_key和duration）
                )
                logger.info(
        ...
        logger.debug(
            f"[FeiShu] Sending message: msg_type={msg_type}, content={content_json[:200]}"
        )
        ...
        if can_reply:
            ...
            res = requests.post(
                url=url, headers=headers, params=params, json=data, timeout=(5, 10)
            )
        res = res.json()
        if res.get("code") == 0:
    ...
    def fetch_access_token(self) -> str:
        ...
        req_body = {"app_id": self.feishu_app_id, "app_secret": self.feishu_app_secret}
        ...
        if response.status_code == 200:
            ...
            if res.get("code") != 0:
                logger.error(
                    f"[FeiShu] get tenant_access_token error, code={res.get('code')}, msg={res.get('msg')}"
                )
                return ""
            else:
    ...
    def _get_video_duration(self, file_path: str) -> int:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"[FeiShu] Video duration: {duration_seconds:.2f}s ({duration_ms}ms)"
                )
                return duration_ms
            else:
                logger.warning(
                    f"[FeiShu] Failed to get video duration via ffprobe: {result.stderr}"
                )
                return 0
        except FileNotFoundError:
            logger.warning(
                "[FeiShu] ffprobe not found, video duration will be 0. Install ffmpeg to fix this."
            )
            return 0
        except Exception as e:
    ...
    def _upload_video_url(self, video_url, access_token):
        ...
        try:
            ...
            if video_url.startswith("file://"):
                ...
                if response.status_code != 200:
                    logger.error(
                        f"[FeiShu] download video failed, status={response.status_code}"
                    )
                    return None
                ...
                logger.info(
                    f"[FeiShu] Video downloaded, size={len(response.content)} bytes"
                )
            ...
            logger.info(
                f"[FeiShu] Uploading video: file_name={file_name}, duration={duration}ms"
            )
            ...
            if temp_file and os.path.exists(temp_file):
                try:
                    ...
                    logger.warning(
                        f"[FeiShu] Failed to remove temp file {temp_file}: {e}"
                    )
    ...
    def _upload_file_url(self, file_url, access_token):
        ...
        if file_url.startswith("file://"):
            ...
            file_type = file_type_map.get(
                file_ext, "stream"
            )  # Default to stream for other types
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"[FeiShu] download file failed, status={response.status_code}"
                )
                return None
            ...
            with open(temp_name, "rb") as file:
                upload_response = requests.post(
                    upload_url, files={"file": file}, data=data, headers=headers
                )
                logger.info(f"[FeiShu] upload file, res={upload_response.content}")
