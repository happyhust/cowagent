    ...
    def __init__(self, event: dict, is_group=False, access_token=None):
        ...
        if msg_type == "text":
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"[FeiShu] Downloaded single image, key={image_key}, path={image_path}"
                )
                self.content = image_path
                self.image_path = image_path  # 保存图片路径
            else:
                logger.error(
                    f"[FeiShu] Failed to download single image, key={image_key}, status={response.status_code}"
                )
            ...
            logger.info(
                f"[FeiShu] Post message - title: '{title}', content_list length: {len(content_list)}"
            )
            ...
            for block in content_list:
                ...
                for element in block:
                    ...
                    logger.debug(
                        f"[FeiShu] Element tag: {element_tag}, element: {element}"
                    )
                    if element_tag == "img":
                        # 找到图片元素
            ...
            logger.info(
                f"[FeiShu] Parsed - images: {len(image_keys)}, text_parts: {text_parts}"
            )
            ...
            if image_keys:
                    ...
                    for image_key, image_path in self.image_paths.items():
                        ...
                        if response.status_code == 200:
                            with open(image_path, "wb") as f:
                                f.write(response.content)
                            logger.info(
                                f"[FeiShu] Image downloaded from post message, key={image_key}, path={image_path}"
                            )
                        else:
                            logger.error(
                                f"[FeiShu] Failed to download image from post, key={image_key}, status={response.status_code}"
                            )
                ...
                self.content = "\n".join(content_parts)
                logger.info(
                    f"[FeiShu] Received post message with {len(image_keys)} image(s) and text: {self.content}"
                )
            else:
                # 纯文本富文本消息
                self.content = (
                ...
                logger.info(
                    f"[FeiShu] Received post message (text only): {self.content}"
                )
        elif msg_type == "file":
            ...
            self.content = (
                TmpDir().path() + file_key + "." + utils.get_path_suffix(file_name)
            )
            ...
            def _download_file():
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"[FeiShu] Failed to download file, key={file_key}, res={response.text}"
                    )

            self._prepare_fn = _download_file
        else:
            raise NotImplementedError(
                "Unsupported message type: Type:{} ".format(msg_type)
            )
