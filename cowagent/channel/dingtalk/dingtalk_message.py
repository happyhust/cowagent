    ...
    def __init__(self, event: ChatbotMessage, image_download_handler):
        ...
        if self.message_type == "text":
            ...
            if self.message_type == "picture" and len(image_list) > 0:
                ...
                download_url = image_download_handler.get_image_download_url(
                    download_code
                )
                ...
                logger.info(
                    f"[DingTalk] Received richText with {len(image_paths)} image(s): {self.content}"
                )
            else:
                self.ctype = ContextType.IMAGE
                self.content = "[未找到图片]"
                logger.debug(
                    f"[DingTalk] messageType: {self.message_type}, imageList isEmpty"
                )
...
def download_image_file(image_url, temp_dir):
    ...
    if image_url.startswith("dingtalk://download/"):
        ...
        logger.info(
            f"[DingTalk] Downloading image with downloadCode: {download_code[:20]}..."
        )
        ...
        if not client_id or not client_secret:
            logger.error(
                "[DingTalk] Missing dingtalk_client_id or dingtalk_client_secret"
            )
            return None
        ...
        if len(parts) != 2:
            logger.error(
                f"[DingTalk] Invalid download_code format (expected robot_code:download_code): {download_code[:50]}"
            )
            return None
        ...
        try:
            token_response = requests.post(
                token_url, json=token_body, headers=token_headers, timeout=10
            )
            ...
            if token_response.status_code == 200:
                ...
                if download_response.status_code == 200:
                    ...
                    if not download_url:
                        logger.error(
                            f"[DingTalk] No downloadUrl in response: {download_data}"
                        )
                        return None
                    ...
                    if image_response.status_code == 200:
                        ...
                        logger.info(
                            f"[DingTalk] Image downloaded successfully: {file_path}"
                        )
                        return file_path
                    else:
                        logger.error(
                            f"[DingTalk] Failed to download image from URL: {image_response.status_code}"
                        )
                        return None
                else:
                    logger.error(
                        f"[DingTalk] Failed to get download URL: {download_response.status_code}, {download_response.text}"
                    )
                    return None
            else:
                logger.error(
                    f"[DingTalk] Failed to get access token: {token_response.status_code}, {token_response.text}"
                )
                return None
    ...
    # 普通 HTTP(S) URL
        ...
        try:
            response = requests.get(
                image_url, headers=headers, stream=True, timeout=60 * 5
            )
            if response.status_code == 200:
                ...
                logger.info(
                    f"[Dingtalk] Failed to download image file, {response.content}"
                )
                return None
        except Exception as e:
            logger.error(f"[Dingtalk] Exception downloading image: {e}")
