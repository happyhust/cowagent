...
from cowagent.channel.chat_channel import ChatChannel
from cowagent.common.utils import expand_path
from cowagent.channel.dingtalk.dingtalk_message import DingTalkMessage
...
from cowagent.common.time_check import time_checker
...
class DingTalkChanel(ChatChannel, dingtalk_stream.ChatbotHandler):
    ...
    def __init__(self):
        ...
        logger.debug(
            "[DingTalk] client_id={}, client_secret={} ".format(
    ...
    def startup(self):
        ...
        client.register_callback_handler(
            dingtalk_stream.chatbot.ChatbotMessage.TOPIC, self
        )
        logger.info(
            "[DingTalk] ✅ Stream client initialized, ready to receive messages"
        )
        ...
        while self._running:
            ...
            try:
                ...
                async def _session():
                    async with _ws.connect(uri) as websocket:
                        ...
                        async for raw_message in websocket:
                            ...
                            if (
                                result
                                == dingtalk_stream.DingTalkStreamClient.TAG_DISCONNECT
                            ):
                                break
                ...
                if not self._running:
                    break
                logger.warning(
                    f"[DingTalk] Stream session error: {e}, reconnecting in 3s..."
                )
                for _ in range(30):
                    if not self._running:
                        break
        ...
        try:
            ...
            if response.status_code == 200 and "accessToken" in result:
                ...
                self._access_token_expires_at = (
                    current_time + result.get("expireIn", 7200) - 300
                )
    ...
    def send_single_message(self, user_id: str, content: str, robot_code: str) -> bool:
        ...
        if not access_token:
            logger.error(
                "[DingTalk] Failed to send single message: Access token not available."
            )
            return False

        if not robot_code:
            logger.error(
                "[DingTalk] Cannot send single message: robot_code is required"
            )
            return False
        ...
        logger.info(
            f"[DingTalk] Sending single message to user {user_id} with robot_code {robot_code}"
        )
        try:
            response = requests.post(url, headers=headers, json=data, timeout=10)
            result = response.json()
    ...
    def send_group_message(
        self, conversation_id: str, content: str, robot_code: str = None
    ):
        """
        主动发送群消息
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"[DingTalk] Group message sent successfully to {conversation_id}"
                )
                return True
            else:
    ...
    def upload_media(self, file_path: str, media_type: str = "image") -> str:
        ...
        if file_path.startswith("http://") or file_path.startswith("https://"):
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"[DingTalk] Failed to download file from URL: {file_path}"
                    )
                    return None
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                response = requests.post(
                    url, params=params, files=files, timeout=(5, 60)
                )
                result = response.json()
                ...
                if result.get("errcode") == 0:
                    ...
                    logger.info(
                        f"[DingTalk] Media uploaded successfully, media_id={media_id}"
                    )
                    return media_id
                else:
    ...
    def send_image_with_media_id(
        self, access_token: str, media_id: str, incoming_message, is_group: bool
    ) -> bool:
        """
        发送图片消息（使用 media_id）
    ...
    def send_image_message(
        self, receiver: str, media_id: str, is_group: bool, robot_code: str
    ) -> bool:
        """
        发送图片消息
    ...
    def get_image_download_url(self, download_code: str) -> str:
        ...
        logger.info(
            f"[DingTalk] Successfully got image download URL for code: {download_code}"
        )
    ...
    async def process(self, callback: dingtalk_stream.CallbackMessage):
        try:
            ...
            if create_at:
                ...
                if msg_age_s > 60:
                    logger.warning(
                        f"[DingTalk] stale msg filtered (age={msg_age_s:.0f}s), "
    ...
    def handle_single(self, cmsg: DingTalkMessage):
        ...
        # 处理文件缓存逻辑
        from cowagent.channel.file_cache import get_file_cache
        ...
        if cmsg.ctype == ContextType.IMAGE:
            if hasattr(cmsg, "image_path") and cmsg.image_path:
                ...
                logger.info(
                    f"[DingTalk] Image cached for session {session_id}, waiting for user query..."
                )
            # 单张图片不直接处理，等待用户提问
            return
        ...
        if cmsg.ctype == ContextType.TEXT:
            ...
            if cached_files:
                ...
                logger.info(
                    f"[DingTalk] Attached {len(cached_files)} cached file(s) to user query"
                )
                # 清除缓存
    ...
    def send(self, reply: Reply, context: Context):
        logger.debug(
            f"[DingTalk] send() called with reply.type={reply.type}, content_length={len(str(reply.content))}"
        )
        ...
        if msg is None:
            ...
            logger.info(
                f"[DingTalk] Sending scheduled task message to {receiver} (is_group={is_group})"
            )

            # 使用缓存的 robot_code 或配置的值
            robot_code = self._robot_code or conf().get("dingtalk_robot_code")
            logger.info(
                f"[DingTalk] Using robot_code: {robot_code}, cached: {self._robot_code}, config: {conf().get('dingtalk_robot_code')}"
            )
            ...
            if is_group:
                ...
                logger.info(
                    f"[DingTalk] Sending single message to staff_id: {sender_staff_id}"
                )
                success = self.send_single_message(
                    sender_staff_id, reply.content, robot_code
                )
        ...
        if reply.type == ReplyType.IMAGE_URL:
            ...
            if media_id:
                ...
                if access_token:
                    success = self.send_image_with_media_id(
                        access_token, media_id, incoming_message, isgroup
                    )
                    if not success:
                    ...
                    self.reply_text(
                        "抱歉，图片发送失败（无法获取token）", incoming_message
                    )
            else:
            ...
            if is_video:
                ...
                if media_id:
                    ...
                    success = self._send_file_message(
                        access_token, incoming_message, "sampleFile", msg_param, isgroup
                    )
                    if not success:
        ...
        # 处理文本消息
            ...
            if conf().get("dingtalk_card_enabled"):
                logger.info(
                    "[Dingtalk] sendMsg={}, receiver={}".format(reply, receiver)
                )
                ...
                def reply_with_ai_markdown():
                    button_list, markdown_content = (
                        self.generate_button_markdown_content(context, reply)
                    )
                    self.reply_ai_markdown_button(
                        incoming_message,
                        markdown_content,
                        button_list,
                        "",
...
{reply_text}
                                ...
                                """
        logger.debug(
            f"[Dingtalk] generate_button_markdown_content, button_list={button_list} , markdown_content={markdown_content}"
        )
