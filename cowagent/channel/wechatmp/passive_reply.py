...
from cowagent.channel.wechatmp.wechatmp_message import WeChatMPMessage
from cowagent.common.log import logger
from cowagent.common.utils import split_string_by_utf8_length
        ...
        try:
            ...
            if args.get("encrypt_type") == "aes":
                ...
                if not channel.crypto:
                    raise Exception(
                        "Crypto not initialized, Please set wechatmp_aes_key in config.json"
                    )
            ...
            if msg.type in ["text", "voice", "image"]:
                ...
                if (
                    ...
                    if (
                        msg.type == "voice"
                        and wechatmp_msg.ctype == ContextType.TEXT
                        and conf().get("voice_reply_voice", False)
                    ):
                    ...
                    logger.debug(
                        "[wechatmp] context: {} {} {}".format(
                            context, wechatmp_msg, supported
                        )
                    )
                ...
                try:
                    ...
                    if not channel.cache_dict[
                        from_user
                    ]:  # If popping the message makes the list empty, delete the user entry from cache
                ...
                if reply_type == "text":
                    ...
                    asyncio.run_coroutine_threadsafe(
                        channel.delete_media(media_id), channel.delete_media_loop
                    )
                    logger.info(
