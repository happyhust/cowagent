...
class WechatMPClient(WeChatClient):
    ...
    def clear_quota_v2(self):
        return self.post(
            "clear_quota/v2", params={"appid": self.appid, "appsecret": self.secret}
        )
    ...
    def _request(
        ...
        try:
            ...
            if (
                self.last_clear_quota_time == -1
                or time.time() - self.last_clear_quota_time > 60
            ):
                with self.clear_quota_lock:
                    if (
                        ...
                        logger.debug(
                            "[wechatmp] API quata has been cleard, {}".format(response)
                        )
                return super()._request(method, url_or_endpoint, **kwargs)
            else:
                logger.error(
                    "[wechatmp] last clear quota time is {}, less than 60s, skip clear quota"
                )
                raise e
