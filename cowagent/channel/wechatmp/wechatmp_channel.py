...
from cowagent.channel.wechatmp.common import *
from cowagent.channel.wechatmp.wechatmp_client import WechatMPClient
from cowagent.common.log import logger
from cowagent.common.singleton import singleton
from cowagent.common.utils import split_string_by_utf8_length, remove_markdown_symbol
from cowagent.config import conf

try:
    from cowagent.voice.audio_convert import any_to_mp3, split_audio
except ImportError as e:
    logger.debug(
        "import cowagent.voice.audio_convert failed, voice features will not be supported: {}".format(
            e
        )
    )
...
class WechatMPChannel(ChatChannel):
    ...
    async def delete_media(self, media_id):
        logger.debug(
            "[wechatmp] permanent media {} will be deleted in 10s".format(media_id)
        )
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if self.passive_reply:
            if (
                ...
                or reply.type == ReplyType.INFO
                or reply.type == ReplyType.ERROR
            ):
                reply_text = remove_markdown_symbol(reply.content)
                logger.info(
                    "[wechatmp] text cached, receiver {}\n{}".format(
                        receiver, reply_text
                    )
                )
                ...
                try:
                    ...
                    if len(files) > 1:
                        logger.info(
                            "[wechatmp] voice too long {}s > 60s , split into {} parts".format(
                    ...
                    for path in files:
                        ...
                        try:
                            with open(path, "rb") as f:
                                ...
                                logger.debug(
                                    "[wechatmp] upload voice response: {}".format(
                                        response
                                    )
                                )
                        ...
                        logger.info(
                            "[wechatmp] voice uploaded, receiver {}, media_id {}".format(
                                receiver, media_id
                            )
                        )
                ...
                filename = (
                    receiver + "-" + str(context["msg"].msg_id) + "." + image_type
                )
                content_type = "image/" + image_type
                try:
                    response = self.client.material.add(
                        "image", (filename, image_storage, content_type)
                    )
                    logger.debug(
                        "[wechatmp] upload image response: {}".format(response)
                    )
                except WeChatClientException as e:
                ...
                media_id = response["media_id"]
                logger.info(
                    "[wechatmp] image uploaded, receiver {}, media_id {}".format(
                        receiver, media_id
                    )
                )
                ...
                video_type = "mp4"
                filename = (
                    receiver + "-" + str(context["msg"].msg_id) + "." + video_type
                )
                content_type = "video/" + video_type
                try:
                    response = self.client.material.add(
                        "video", (filename, video_storage, content_type)
                    )
                    logger.debug(
                        "[wechatmp] upload video response: {}".format(response)
                    )
                except WeChatClientException as e:
                ...
                media_id = response["media_id"]
                logger.info(
                    "[wechatmp] video uploaded, receiver {}, media_id {}".format(
                        receiver, media_id
                    )
                )
            ...
            if (
                ...
                if len(texts) > 1:
                    logger.info(
                        "[wechatmp] text too long, split into {} parts".format(
                            len(texts)
                        )
                    )
                ...
                logger.info(
                    "[wechatmp] Do send text to {}: {}".format(receiver, reply_text)
                )
            elif reply.type == ReplyType.VOICE:
                try:
                    ...
                    if file_type == ".mp3":
                        ...
                        file_type = "audio/mpeg"
                    logger.info(
                        "[wechatmp] file_name: {}, file_type: {} ".format(
                            file_name, file_type
                        )
                    )
                    media_ids = []
                    ...
                    for path in files:
                        ...
                        response = self.client.media.upload(
                            "voice",
                            (os.path.basename(path), open(path, "rb"), file_type),
                        )
                        logger.debug(
                            "[wechatcom] upload voice response: {}".format(response)
                        )
    ...
    def _success_callback(
        self, session_id, context, **kwargs
    ):  # 线程异常结束时的回调函数
        logger.debug(
    ...
    def _fail_callback(
        self, session_id, exception, context, **kwargs
    ):  # 线程异常结束时的回调函数
        logger.exception(
