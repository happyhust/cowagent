...
from cowagent.channel.qq.qq_message import QQMessage
...
from cowagent.common.ws_client_compat import websocket_app_run_forever
...
class QQChannel(ChatChannel):
    ...
    def _start_ws(self):
        ...
        def _on_close(ws, close_status_code, close_msg):
            logger.warning(
                f"[QQ] WebSocket closed: status={close_status_code}, msg={close_msg}"
            )
            self._connected = False
    ...
    def _send_resume(self):
        ...
        logger.debug(
            f"[QQ] Resume sent: session_id={self._session_id}, seq={self._last_seq}"
        )
    ...
    def _handle_ws_message(self, data: dict):
        ...
        if op == OP_HELLO:
            ...
            logger.debug(
                f"[QQ] Received Hello, heartbeat_interval={heartbeat_interval}ms"
            )
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if not msg:
            ...
            self._active_send_text(
                reply.content if reply.type == ReplyType.TEXT else str(reply.content),
                receiver,
                is_group,
            )
            return
        ...
        if reply.type == ReplyType.TEXT:
            ...
            logger.warning(
                f"[QQ] Unsupported reply type: {reply.type}, falling back to text"
            )
    ...
    def _build_msg_url_and_base_body(
        self, msg: QQMessage, event_type: str, msg_id: str
    ):
        ...
        if event_type == "GROUP_AT_MESSAGE_CREATE":
            ...
            user_openid = (
                msg._rawmsg.get("author", {}).get("user_openid", "") or msg.from_user_id
            )
    ...
    def _post_message(self, url: str, body: dict, event_type: str):
        try:
            resp = requests.post(
                url, json=body, headers=self._get_auth_headers(), timeout=10
            )
            if resp.status_code in (200, 201, 202, 204):
                logger.info(f"[QQ] Message sent successfully: event_type={event_type}")
            else:
                logger.error(
                    f"[QQ] Failed to send message: status={resp.status_code}, "
                    f"body={resp.text}"
                )
        except Exception as e:
    ...
    def _upload_rich_media(
        self, file_url: str, file_type: int, msg: QQMessage, event_type: str
    ) -> str:
        """
        ...
        if event_type == "GROUP_AT_MESSAGE_CREATE":
            ...
            logger.warning(
                f"[QQ] Rich media upload not supported for event_type: {event_type}"
            )
            return ""
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                ...
                logger.info(
                    f"[QQ] Rich media uploaded: file_type={file_type}, "
                    f"file_uuid={data.get('file_uuid', '')}"
                )
                return file_info
            else:
                logger.error(
                    f"[QQ] Rich media upload failed: status={resp.status_code}, "
                    f"body={resp.text}"
                )
                return ""
        except Exception as e:
    ...
    def _upload_rich_media_base64(
        self, file_path: str, file_type: int, msg: QQMessage, event_type: str
    ) -> str:
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                ...
                file_info = data.get("file_info", "")
                logger.info(
                    f"[QQ] Rich media uploaded (base64): file_type={file_type}, "
                    f"file_uuid={data.get('file_uuid', '')}"
                )
                return file_info
            else:
                logger.error(
                    f"[QQ] Rich media upload (base64) failed: status={resp.status_code}, "
                    f"body={resp.text}"
                )
                return ""
        except Exception as e:
    ...
    def _send_media_msg(
        self, file_info: str, msg: QQMessage, event_type: str, msg_id: str
    ):
    ...
    def _send_image(
        self, img_path_or_url: str, msg: QQMessage, event_type: str, msg_id: str
    ):
    ...
    def _send_file(
        self, file_path_or_url: str, msg: QQMessage, event_type: str, msg_id: str
    ):
        """Send file reply."""
        if event_type not in ("GROUP_AT_MESSAGE_CREATE", "C2C_MESSAGE_CREATE"):
    ...
    def _send_media(
        ...
        if path_or_url.startswith(("http://", "https://")):
            ...
            file_info = self._upload_rich_media_base64(
                path_or_url, file_type, msg, event_type
            )
        else:
