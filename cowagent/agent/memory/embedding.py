...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    def _call_api(self, input_data):
        ...
        data = {"input": input_data, "model": self.model}
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to OpenAI API at {url}. Please check your network connection and api_base configuration. Error: {str(e)}"
            )
        except requests.exceptions.Timeout as e:
            raise TimeoutError(
                f"OpenAI API request timed out after 10s. Please check your network connection. Error: {str(e)}"
            )
...
def create_embedding_provider(
    ...
    if provider not in ("openai", "linkai"):
        raise ValueError(
            f"Unsupported embedding provider: {provider}. Use 'openai' or 'linkai'."
        )
