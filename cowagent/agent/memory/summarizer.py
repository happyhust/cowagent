...
class MemoryFlushManager:
    ...
    def __init__(
        ...
        self._trim_flushed_hashes: set = (
            set()
        )  # Content hashes of already-flushed messages
        self._last_flushed_content_hash: str = (
            ""  # Content hash at last flush, for daily dedup
        )

    def get_today_memory_file(
        self, user_id: Optional[str] = None, ensure_exists: bool = False
    ) -> Path:
    ...
    def flush_from_messages(
        ...
        try:
            ...
            thread.start()
            logger.info(
                f"[MemoryFlush] Async flush dispatched (reason={reason}, msgs={len(snapshot)})"
            )
            return True

        except Exception as e:
            logger.warning(
                f"[MemoryFlush] Failed to dispatch flush (reason={reason}): {e}"
            )
            return False
    ...
    def _flush_worker(
        ...
        try:
            ...
            if not summary or not summary.strip() or summary.strip() == "无":
                logger.info(
                    f"[MemoryFlush] No valuable content to flush (reason={reason})"
                )
                return
            ...
            if reason == "overflow":
                header = (
                    f"## Context Overflow Recovery ({datetime.now().strftime('%H:%M')})"
                )
                note = (
                    "The following conversation was trimmed due to context overflow:\n"
                )
            elif reason == "trim":
            ...
            logger.info(
                f"[MemoryFlush] Wrote to {daily_file.name} (reason={reason}, chars={len(summary)})"
            )
    ...
    def create_daily_summary(
        ...
        content = "".join(
            self._extract_text_from_content(m.get("content", "")) for m in messages
        )
        content_hash = hashlib.md5(content.encode("utf-8")).hexdigest()
        if content_hash == self._last_flushed_content_hash:
            logger.debug(
                "[MemoryFlush] Daily summary skipped: no new content since last flush"
            )
            return False
    ...
    def _summarize_messages(self, messages: List[Dict], max_messages: int = 0) -> str:
        ...
        if self.llm_model:
            ...
            logger.info(
                "[MemoryFlush] No LLM model available, using rule-based fallback"
            )

        return self._extract_summary_fallback(messages, max_messages)

    def _format_conversation_for_summary(
        self, messages: List[Dict], max_messages: int = 0
    ) -> str:
    ...
    def _call_llm_for_summary(self, conversation_text: str) -> str:
        ...
        from cowagent.agent.protocol.models import LLMRequest

        request = LLMRequest(
            messages=[
                {
                    "role": "user",
                    "content": SUMMARIZE_USER_PROMPT.format(
...
def ensure_daily_memory_file(
    workspace_dir: Path, user_id: Optional[str] = None
) -> Path:
    """
