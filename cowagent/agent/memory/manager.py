...
from cowagent.agent.memory.chunker import TextChunker
from cowagent.agent.memory.embedding import create_embedding_provider, EmbeddingProvider
from cowagent.agent.memory.summarizer import (
    MemoryFlushManager,
...
class MemoryManager:
    ...
    def __init__(
        ...
        self.flush_manager = MemoryFlushManager(
            workspace_dir=workspace_dir, llm_model=llm_model
        )
    ...
    async def search(
        ...
        if self.embedding_provider:
            try:
                ...
                logger.info(
                    f"[MemoryManager] Vector search found {len(vector_results)} results for query: {query}"
                )
            except Exception as e:
                from cowagent.common.log import logger
        ...
        logger.info(
            f"[MemoryManager] Keyword search found {len(keyword_results)} results for query: {query}"
        )
    ...
    async def _sync_file(
        self, file_path: Path, source: str, scope: str, user_id: Optional[str]
    ):
        """Sync a single file"""
        # Compute file hash
