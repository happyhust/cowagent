    ...
    def run(
        ...
        def on_event(event: dict):
            ...
            if event_type == "message_update":
                ...
                arguments = state.pending_tool_arguments.pop(
                    tool_call_id, data.get("arguments", {})
                )
        ...
        logger.info(
            f"[ChatService] Starting agent run: session={session_id}, query={query[:80]}"
        )
        ...
        try:
            ...
            if len(executor.messages) == 0:
                with agent.messages_lock:
                    ...
                    logger.info(
                        "[ChatService] Cleared agent message history after executor recovery"
                    )
            raise
