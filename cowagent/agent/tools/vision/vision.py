...
class Vision(BaseTool):
    ...
    def _call_with_fallback(
        ...
        for i, provider in enumerate(providers):
            ...
            try:
                logger.debug(
                    f"[Vision] Trying provider '{provider.name}' "
                ...
                errors.append(
                    f"[{provider.name}/{use_model}] Request timed out after {DEFAULT_TIMEOUT}s"
                )
                ...
                errors.append(f"[{provider.name}/{use_model}] {e}")
                logger.error(
                    f"[Vision] Provider '{provider.name}' unexpected error: {e}",
                    exc_info=True,
                )
    ...
    _PROVIDER_BUILDERS = [
        _build_custom_model_provider,
        _build_openai_provider,
        _build_linkai_provider,
    ]
    ...
    def _call_api(
        self, provider: VisionProvider, model: str, question: str, image_content: dict
    ) -> ToolResult:
        """
