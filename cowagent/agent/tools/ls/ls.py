...
class Ls(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        if os.path.abspath(absolute_path) == os.path.abspath(workspace):
        ...
        try:
            ...
            if not results:
                return ToolResult.success(
                    {"message": "(empty directory)", "entries": []}
                )

            # Format output
            raw_output = "\n".join(results)
            truncation = truncate_head(
                raw_output, max_lines=999999
            )  # Only limit by bytes
            ...
            return ToolResult.fail(
                f"Error: Permission denied reading directory: {path}"
            )
