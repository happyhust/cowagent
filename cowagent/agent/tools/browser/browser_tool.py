...
from cowagent.agent.tools.browser.browser_service import BrowserService
...
class BrowserTool(BaseTool):
    ...
    def _do_fill(self, args: Dict[str, Any]) -> ToolResult:
        ...
        result = self._get_service().fill(
            text, ref=ref, selector=selector, timeout=timeout
        )
        if "error" in result:
            return ToolResult.fail(result["error"])
        return ToolResult.success(
    ...
    def _do_select(self, args: Dict[str, Any]) -> ToolResult:
        ...
        result = self._get_service().select(
            value, ref=ref, selector=selector, timeout=timeout
        )
        if "error" in result:
            return ToolResult.fail(result["error"])
