...
class WebSearch(BaseTool):
    ...
    def _search_bocha(
        ...
        if response.status_code == 401:
            return ToolResult.fail(
                "Error: Invalid BOCHA_API_KEY. Please check your API key."
            )
        if response.status_code == 403:
            return ToolResult.fail(
                "Error: Bocha API - insufficient balance. Please top up at https://open.bocha.cn"
            )
        if response.status_code == 429:
            return ToolResult.fail(
                "Error: Bocha API rate limit reached. Please try again later."
            )
        if response.status_code != 200:
            return ToolResult.fail(
                f"Error: Bocha API returned HTTP {response.status_code}"
            )
    ...
    def _search_linkai(self, query: str, count: int, freshness: str) -> ToolResult:
        ...
        if response.status_code == 401:
            return ToolResult.fail(
                "Error: Invalid LINKAI_API_KEY. Please check your API key."
            )
        if response.status_code != 200:
            return ToolResult.fail(
                f"Error: LinkAI API returned HTTP {response.status_code}"
            )
