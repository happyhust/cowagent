...
from cowagent.agent.tools.utils.truncate import (
    truncate_tail,
    format_size,
    DEFAULT_MAX_LINES,
    DEFAULT_MAX_BYTES,
...
class Bash(BaseTool):
    ...
    name: str = "bash"
    description: str = f"""Execute a bash command in the current working directory. Returns stdout and stderr. Output is truncated to last {
        DEFAULT_MAX_LINES
    } lines or {
        DEFAULT_MAX_BYTES // 1024
    }KB (whichever is hit first). If truncated, full output is saved to a temp file.
{
        '''
        ...
        try:
            ...
            if os.path.exists(env_file):
                try:
                    ...
                    logger.debug(
                        f"[Bash] Loaded {len(dotenv_vars)} variables from {env_file}"
                    )
                except ImportError:
                    logger.debug(
                        "[Bash] python-dotenv not installed, skipping .env loading"
                    )
            ...
            if hasattr(os, "getuid"):
                ...
                logger.debug(
                    f"[Bash] Process User: {os.environ.get('USERNAME', os.environ.get('USER', 'unknown'))}"
                )
            ...
            if result.returncode == 126 and not result.stdout and not result.stderr:
                logger.warning(
                    f"[Bash] Exit 126 with no output - trying alternative execution method"
                )
                ...
                try:
                    ...
                    if len(parts) > 0:
                        logger.info(
                            f"[Bash] Retrying with argument list: {parts[:3]}..."
                        )
                        ...
                        logger.debug(
                            f"[Bash] Retry exit code: {retry_result.returncode}, stdout: {len(retry_result.stdout)}, stderr: {len(retry_result.stderr)}"
                        )
                        ...
                        if (
                            ...
                            if (
                                "openai-image-vision" in command
                                or "vision.sh" in command
                            ):
                                ...
                                logger.info(
                                    f"[Bash] Converted exit 126 to user-friendly image error message for vision skill"
                                )
            ...
            if total_bytes > DEFAULT_MAX_BYTES:
                ...
                with tempfile.NamedTemporaryFile(
                    mode="w", delete=False, suffix=".log", prefix="bash-"
                ) as f:
                    f.write(output)
