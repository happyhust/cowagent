...
class SchedulerTool(BaseTool):
    ...
    params: dict = {
        ...
        "properties": {
            ...
            "name": {"type": "string", "description": "任务名称 (用于 create 操作)"},
            "message": {
                "type": "string",
    ...
    def _create_task(self, **kwargs) -> str:
        ...
        if not schedule:
            return (
                f"错误: 无效的调度配置 - type: {schedule_type}, value: {schedule_value}"
            )
        ...
        if (
            msg
            and hasattr(msg, "sender_staff_id")
            and not context.get("isgroup", False)
        ):
        ...
        receiver_desc = (
            task_data["action"]["receiver_name"] or task_data["action"]["receiver"]
        )
    ...
    def _list_tasks(self, **kwargs) -> str:
        ...
        for task in tasks:
            ...
            next_run_str = (
                datetime.fromisoformat(next_run).strftime("%m-%d %H:%M")
                if next_run
                else "未知"
            )
    ...
    def _get_task(self, **kwargs) -> str:
        ...
        next_run = task.get("next_run_at")
        next_run_str = (
            datetime.fromisoformat(next_run).strftime("%Y-%m-%d %H:%M:%S")
            if next_run
            else "未知"
        )
        last_run = task.get("last_run_at")
        last_run_str = (
            datetime.fromisoformat(last_run).strftime("%Y-%m-%d %H:%M:%S")
            if last_run
            else "从未执行"
        )
