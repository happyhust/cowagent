...
class SchedulerService:
        ...
        for task in tasks:
            try:
                ...
                if self._is_task_due(task, now):
                    logger.info(
                        f"[Scheduler] Executing task: {task['id']} - {task['name']}"
                    )
                    ...
                    if next_run:
                        ...
                        logger.info(
                            f"[Scheduler] One-time task completed and removed: {task['id']}"
                        )
            except Exception as e:
    ...
    def _is_task_due(self, task: dict, now: datetime) -> bool:
        ...
        if not next_run_str:
            ...
            if next_run:
                self.task_store.update_task(
                    task["id"], {"next_run_at": next_run.isoformat()}
                )
                return False
            return False
        ...
        try:
            ...
            if next_run < now:
                ...
                if time_diff > 300:  # 5 minutes
                    logger.warning(
                        f"[Scheduler] Task {task['id']} is overdue by {int(time_diff)}s, skipping and scheduling next run"
                    )
                    ...
                    if schedule.get("type") == "once":
                        self.task_store.delete_task(task["id"])
                        logger.info(
                            f"[Scheduler] One-time task {task['id']} expired, removed"
                        )
                        return False
                    ...
                    if next_next_run:
                        self.task_store.update_task(
                            task["id"], {"next_run_at": next_next_run.isoformat()}
                        )
                        logger.info(
                            f"[Scheduler] Rescheduled task {task['id']} to {next_next_run}"
                        )
                    return False
    ...
    def _calculate_next_run(
        self, task: dict, from_time: datetime
    ) -> Optional[datetime]:
        """
    ...
    def _execute_task(self, task: dict):
        ...
        try:
            ...
            self.task_store.update_task(
                task["id"],
                {"last_error": str(e), "last_error_at": datetime.now().isoformat()},
            )
