...
def _execute_agent_task(task: dict, agent_bridge):
    ...
    try:
        ...
        if not task_description:
            logger.error(
                f"[Scheduler] Task {task['id']}: No task_description specified"
            )
            return
        ...
        if channel_type == "dingtalk":
            logger.warning(
                f"[Scheduler] Task {task['id']}: DingTalk channel does not support scheduled messages (Stream mode limitation). Task will execute but message cannot be sent."
            )

        logger.info(
            f"[Scheduler] Task {task['id']}: Executing agent task '{task_description}'"
        )
        ...
        try:
            ...
            reply = agent_bridge.agent_reply(
                task_description, context=context, on_event=None, clear_history=False
            )

            if reply and reply.content:
                # Send the reply via channel
                from cowagent.channel.channel_factory import create_channel

                try:
                    channel = create_channel(channel_type)
                    if channel:
                        # For web channel, register request_id
                        if channel_type == "web" and hasattr(
                        ...
                        logger.error(
                            f"[Scheduler] Failed to create channel: {channel_type}"
                        )
                except Exception as e:
                    logger.error(f"[Scheduler] Failed to send result: {e}")
            else:
                logger.error(
                    f"[Scheduler] Task {task['id']}: No result from cowagent.agent execution"
                )
...
def _execute_send_message(task: dict, agent_bridge):
    ...
    try:
        ...
        if channel_type == "web":
            ...
            context["request_id"] = request_id
            logger.debug(
                f"[Scheduler] Generated request_id for web channel: {request_id}"
            )
        elif channel_type == "feishu":
            ...
            context["msg"] = None
            logger.debug(
                f"[Scheduler] Feishu: receive_id_type={context['receive_id_type']}, is_group={is_group}, receiver={receiver}"
            )
        elif channel_type == "dingtalk":
            ...
            if not is_group:
                ...
                if sender_staff_id:
                    context["dingtalk_sender_staff_id"] = sender_staff_id
                    logger.debug(
                        f"[Scheduler] DingTalk single chat: sender_staff_id={sender_staff_id}"
                    )
                else:
                    logger.warning(
                        f"[Scheduler] Task {task['id']}: DingTalk single chat message missing sender_staff_id"
                    )
        elif channel_type == "wecom_bot":
            context["msg"] = None
...
def _execute_tool_call(task: dict, agent_bridge):
    ...
    try:
        ...
        # Execute tool
        logger.info(
            f"[Scheduler] Task {task['id']}: Executing tool '{tool_name}' with params {tool_params}"
        )
...
def _execute_skill_call(task: dict, agent_bridge):
    ...
    try:
        ...
        logger.info(
            f"[Scheduler] Task {task['id']}: Executing skill '{skill_name}' with params {skill_params}"
        )
        ...
        try:
            # Don't clear history - scheduler tasks use isolated session_id so they won't pollute user conversations
            reply = agent_bridge.agent_reply(
                query, context=context, on_event=None, clear_history=False
            )
            ...
            if reply and reply.content:
                ...
                logger.info(
                    f"[Scheduler] Task {task['id']} executed: skill result sent to {receiver}"
                )
            else:
                logger.error(
                    f"[Scheduler] Task {task['id']}: No result from skill execution"
                )
...
def attach_scheduler_to_tool(tool, context: Context = None):
    ...
    if context:
        ...
        channel_type = context.get("channel_type") or conf().get(
