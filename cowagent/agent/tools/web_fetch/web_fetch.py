...
from cowagent.agent.tools.utils.truncate import truncate_head, format_size
...
ALL_DOC_SUFFIXES = (
    PDF_SUFFIXES | WORD_SUFFIXES | TEXT_SUFFIXES | SPREADSHEET_SUFFIXES | PPT_SUFFIXES
)
...
class WebFetch(BaseTool):
    ...
    def _fetch_webpage(self, url: str) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.fail(
                f"Error: HTTP {e.response.status_code} for URL: {url}"
            )
    ...
    def _fetch_document(self, url: str) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.fail(
                f"Error: Download timed out after {DEFAULT_TIMEOUT}s"
            )
        except requests.ConnectionError:
            return ToolResult.fail(f"Error: Failed to connect to {parsed.netloc}")
        except requests.HTTPError as e:
    ...
    def _parse_ppt(self, file_path: str) -> str:
        ...
        for slide_num, slide in enumerate(prs.slides, 1):
            ...
            if slide_texts:
                text_parts.append(
                    f"--- Slide {slide_num}/{len(prs.slides)} ---\n"
                    + "\n".join(slide_texts)
                )
    ...
    def _handle_download_by_content_type(
        ...
        if detected_suffix and detected_suffix in ALL_DOC_SUFFIXES:
            ...
            return self._fetch_document(
                url
                if _get_url_suffix(url) in ALL_DOC_SUFFIXES
                else self._rewrite_url_with_suffix(url, detected_suffix)
            )
        return ToolResult.fail(
            f"Error: URL returned binary content ({content_type}), not a supported document type"
        )
