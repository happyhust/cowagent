...
class ToolManager:
    ...
    def _load_tools_from_init(self) -> bool:
        ...
        try:
            ...
            tools_package = importlib.import_module("cowagent.agent.tools")
            ...
            logger.warning("Could not import cowagent.agent.tools package")
            return False
        except Exception as e:
                        ...
                        if (
                            ...
                            try:
                                ...
                                if attr_name in ["MemorySearchTool", "MemoryGetTool"]:
                                    logger.debug(
                                        f"Skipped tool {attr_name} (requires memory_manager)"
                                    )
                                    continue
            ...
            if missing_tools:
                for tool_name in missing_tools:
                    if tool_name == "browser":
                        ...
                        logger.warning(
                            f"[ToolManager] Tool '{tool_name}' is configured but could not be loaded."
                        )
