...
class Read(BaseTool):
    ...
    def _return_file_metadata(
        self, absolute_path: str, file_type: str, file_size: int
    ) -> ToolResult:
        """
    ...
    def _read_text(
        ...
        try:
            ...
            return ToolResult.fail(
                f"Error: File is not a valid text file (encoding error): {display_path}"
            )
        except Exception as e:
    ...
    def _read_office(
        ...
        if truncation.truncated:
            ...
            user_limited_lines is not None
            and start_line + user_limited_lines < total_lines
        ):
