...
from cowagent.agent.protocol.message_utils import (
...
class AgentStreamExecutor:
    ...
    def __init__(
        ...
        self.tools = (
            {tool.name: tool for tool in tools} if isinstance(tools, list) else tools
        )
    ...
    def _emit_event(self, event_type: str, data: dict = None):
        ...
        if self.on_event:
            try:
                self.on_event(
                    {"type": event_type, "timestamp": time.time(), "data": data or {}}
                )
    ...
    def _check_consecutive_failures(
        ...
        if same_args_calls >= 5:
            return (
                True,
                f"工具 '{tool_name}' 使用相同参数已被调用 {same_args_calls} 次，停止执行以防止无限循环。如果需要查看配置，结果已在之前的调用中返回。",
                False,
            )
        ...
        if same_args_failures >= 3:
            return (
                True,
                f"工具 '{tool_name}' 使用相同参数连续失败 {same_args_failures} 次，停止执行以防止无限循环",
                False,
            )
        ...
        if same_tool_failures >= 6:
            return (
                True,
                f"工具 '{tool_name}' 连续失败 {same_tool_failures} 次（使用不同参数），停止执行以防止无限循环",
                False,
            )
    ...
    def run_stream(self, user_message: str) -> str:
        ...
        try:
                ...
                try:
                    for tool_call in tool_calls:
                        ...
                        if turn > 2:
                            ...
                            repeat_count = sum(
                                1
                                for name, ahash, _ in self.tool_failure_history[-10:]
                                if name == tool_call["name"]
                                and ahash == self._hash_args(tool_call["arguments"])
                            )
                            if repeat_count >= 3:
                                logger.warning(
                        ...
                        if result.get("status") == "success" and isinstance(
                            ...
                            if result_data.get("type") == "file_to_send":
                                ...
                                logger.info(
                                    f"📎 检测到待发送文件: {result_data.get('file_name', result_data.get('path'))}"
                                )
                        ...
                        logger.info(
                            f"  {status_emoji} {tool_call['name']} ({result.get('execution_time', 0):.2f}s): {result_str[:200]}{'...' if len(result_str) > 200 else ''}"
                        )
                        ...
                        if is_error:
                            ...
                            result_content = (
                                f"Error: {result.get('result', 'Unknown error')}"
                            )
                        elif isinstance(result.get("result"), dict):
                            # For dict results, use JSON format
                            result_content = json.dumps(
                                result.get("result"), ensure_ascii=False
                            )
                        ...
                        if len(result_content) > MAX_CURRENT_TURN_RESULT_CHARS:
                            ...
                            logger.info(
                                f"📎 Truncated tool result for '{tool_call['name']}': {truncated_len} -> {MAX_CURRENT_TURN_RESULT_CHARS} chars"
                            )
                    ...
                    if tool_result_blocks:
                        ...
                        if turn >= 3 and len(tool_calls) > 0:
                            ...
                            for name, ahash, success in reversed(
                                self.tool_failure_history[-10:]
                            ):
                        ...
                        logger.warning(
                            "⚠️ Tool execution interrupted, adding error results to maintain message history"
                        )
                        emergency_blocks = []
                        for tool_call in tool_calls:
                        ...
                        self.messages.append(
                            {"role": "user", "content": emergency_blocks}
                        )
            ...
            if turn >= self.max_turns:
                ...
                try:
                    ...
                    if summary_response:
                        ...
                        logger.info(
                            f"💭 Summary: {summary_response[:150]}{'...' if len(summary_response) > 150 else ''}"
                        )
                    else:
                    ...
                    if (
                        ...
                        logger.debug(
                            "[Agent] Removed injected max-steps prompt from message history"
                        )
            ...
            final_response = (
                final_response.strip() if final_response else final_response
            )
    ...
    def _call_llm_stream(
        ...
        try:
            ...
            for chunk in stream:
                ...
                if isinstance(chunk, dict) and chunk.get("error"):
                    ...
                    if is_overflow:
                        ...
                        raise Exception(
                            f"[CONTEXT_OVERFLOW] {error_msg} (Status: {status_code})"
                        )
                    else:
                ...
                if isinstance(chunk, dict) and chunk.get("choices"):
                    ...
                    if content_delta:
                        ...
                        if (
                            ...
                            self._emit_event(
                                "message_update", {"delta": filtered_delta}
                            )
            ...
            if is_retryable and retry_count < max_retries:
                ...
                if retry_count >= max_retries:
                    logger.error(
                        f"❌ LLM API error after {max_retries} retries: {e}",
                        exc_info=True,
                    )
                else:
                    logger.error(
                        f"❌ LLM call error (non-retryable): {e}", exc_info=True
                    )
                raise
        ...
        for idx in sorted(tool_calls_buffer.keys()):
            ...
            tool_calls.append(
                {"id": tool_id, "name": tc["name"], "arguments": arguments}
            )

        # Check for empty response and retry once if enabled
        if retry_on_empty and not full_content and not tool_calls:
            logger.warning(
                f"⚠️  LLM returned empty response (stop_reason: {stop_reason}), retrying once..."
            )
            self._emit_event(
                "message_end",
                {
        ...
        self._emit_event(
            "message_end", {"content": full_content, "tool_calls": tool_calls}
        )
        ...
        try:
            ...
            if tool_name == "bash" and result.status == "success":
                ...
                if "init_skill.py" in command and self.agent.skill_manager:
                    ...
                    logger.info(
                        f"Skills refreshed! Now have {len(self.agent.skill_manager.skills)} skills"
                    )
    ...
    def _truncate_historical_tool_results(self):
        ...
        for i in range(current_turn_start):
            ...
            for block in content:
                ...
                if (
                    ...
                    and len(result_str) > MAX_HISTORY_RESULT_CHARS
                ):
        ...
        if truncated_count > 0:
            logger.info(
                f"📎 Truncated {truncated_count} historical tool result(s) to {MAX_HISTORY_RESULT_CHARS} chars"
            )
        ...
        for msg in self.messages:
            ...
            for block in content:
                ...
                if block.get("type") == "tool_result":
                    ...
                    if (
                        isinstance(result_str, str)
                        and len(result_str) > AGGRESSIVE_LIMIT
                    ):
                        block["content"] = (
                    ...
                    if len(input_str) > AGGRESSIVE_LIMIT:
                        ...
                        for key, val in block["input"].items():
                            if isinstance(val, str) and len(val) > 1000:
                                block["input"][key] = (
                                    val[:1000] + f"... [truncated {len(val)} chars]"
                                )
                        truncated += 1
    ...
    def _trim_messages(self):
        ...
        if current_tokens + system_tokens <= max_tokens:
            ...
            if old_count > len(self.messages):
                logger.info(
                    f"   重建消息列表: {old_count} -> {len(self.messages)} 条消息"
                )
            return
