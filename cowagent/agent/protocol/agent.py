...
class Agent:
    def __init__(
        ...
        self.context_reserve_tokens = (
            context_reserve_tokens  # reserve tokens for new requests
        )
        ...
        if enable_skills:
            if skill_manager:
                ...
                try:
                    ...
                    custom_dir = (
                        os.path.join(workspace_dir, "skills") if workspace_dir else None
                    )
                    self.skill_manager = SkillManager(custom_dir=custom_dir)
                    logger.debug(
                        f"Initialized SkillManager with {len(self.skill_manager.skills)} skills"
                    )
    ...
    def get_full_system_prompt(self, skill_filter=None) -> str:
        ...
        """
        try:
            from cowagent.agent.prompt import load_context_files, PromptBuilder

            if self.skill_manager:
                self.skill_manager.refresh_skills()

            context_files = (
                load_context_files(self.workspace_dir) if self.workspace_dir else None
            )

            builder = PromptBuilder(
                workspace_dir=self.workspace_dir or "", language="zh"
            )
            ...
            logger.warning(
                f"Failed to rebuild system prompt, using cached version: {e}"
            )
    ...
    def refresh_skills(self):
        ...
        if self.skill_manager:
            self.skill_manager.refresh_skills()
            logger.info(
                f"Refreshed skills: {len(self.skill_manager.skills)} skills loaded"
            )
        ...
        for tool in self.tools:
            if tool.name == tool_name:
                ...
                if tool.stage == ToolStage.PRE_PROCESS:
                    ...
                    logger.warning(
                        f"Tool {tool_name} is a post-process tool and cannot be called directly."
                    )
                    return None
        return None
        ...
        try:
            ...
            if len(executor.messages) == 0:
                with self.messages_lock:
                    self.messages.clear()
                    logger.info(
                        "[Agent] Cleared Agent message history after executor recovery"
                    )
            raise
