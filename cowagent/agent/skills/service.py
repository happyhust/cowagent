...
class SkillService:
    ...
    def _add_url(self, name: str, payload: dict) -> None:
        ...
        try:
            for file_info in files:
                ...
                if not url or not rel_path:
                    logger.warning(
                        f"[SkillService] add: skip invalid file entry {file_info}"
                    )
                    continue
        ...
        logger.info(
            f"[SkillService] add: skill '{name}' installed via url ({len(files)} files)"
        )
    ...
    def _add_package(self, name: str, payload: dict) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            top_items = [
                item for item in os.listdir(extract_dir) if not item.startswith(".")
            ]
            if len(top_items) == 1:
    ...
    def _download_file(url: str, dest: str):
        ...
        if requests is None:
            raise RuntimeError(
                "requests library is required for downloading skill files"
            )
