...
class SkillManager:
    ...
    def __init__(
        ...
        self.custom_dir = custom_dir or os.path.join(
    ...
    def filter_skills(
        ...
        """
        from cowagent.agent.skills.config import should_include_skill
    ...
    def filter_unavailable_skills(
        ...
        """
        from cowagent.agent.skills.config import (
    ...
    def build_skills_prompt(
        ...
        """
        from cowagent.common.log import logger
        from cowagent.agent.skills.formatter import format_unavailable_skills_for_prompt
        ...
        unavailable, missing_map = self.filter_unavailable_skills(
            skill_filter=skill_filter
        )
        if unavailable:
            unavailable_names = [e.skill.name for e in unavailable]
            logger.debug(
                f"[SkillManager] Unavailable skills (setup needed): {unavailable_names}"
            )
            ...
            skills_info.append(
                {
                    ...
                    "primary_env": entry.metadata.primary_env
                    if entry.metadata
                    else None,
                }
            )
