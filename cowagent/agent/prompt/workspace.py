...
def ensure_workspace(
    workspace_dir: str, create_templates: bool = True
) -> WorkspaceFiles:
    """
    确保工作空间存在，并创建必要的模板文件
    ...
    memory_path = os.path.join(
        workspace_dir, DEFAULT_MEMORY_FILENAME
    )  # MEMORY.md 在根目录
...
def load_context_files(
    workspace_dir: str, files_to_load: Optional[List[str]] = None
) -> List[ContextFile]:
    """
    加载工作空间的上下文文件
        ...
        if filename == DEFAULT_BOOTSTRAP_FILENAME:
            if _is_onboarding_done(workspace_dir):
                try:
                    os.remove(filepath)
                    logger.info(
                        "[Workspace] Auto-removed BOOTSTRAP.md (onboarding already complete)"
                    )
                except Exception:
                    pass
                continue
