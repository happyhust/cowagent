...
def _build_skills_section(
    skill_manager: Any, tools: Optional[List[Any]], language: str
) -> List[str]:
    """构建技能系统section"""
    ...
    try:
        ...
        logger.debug(
            f"[PromptBuilder] Skills prompt length: {len(skills_prompt) if skills_prompt else 0}"
        )
        if skills_prompt:
            ...
            logger.warning(
                "[PromptBuilder] No skills prompt generated - skills_prompt is empty"
            )
...
def _build_memory_section(
    memory_manager: Any, tools: Optional[List[Any]], language: str
) -> List[str]:
    """构建记忆系统section"""
    ...
    if tools:
        ...
        has_memory_tools = any(
            name in ["memory_search", "memory_get"] for name in tool_names
        )
...
def _build_user_identity_section(
    user_identity: Dict[str, str], language: str
) -> List[str]:
    """构建用户身份section"""
...
def _build_cloud_website_section(workspace_dir: str) -> List[str]:
    ...
    try:
        from cowagent.common.cloud_client import build_website_prompt
...
def _build_context_files_section(
    context_files: List[ContextFile], language: str
) -> List[str]:
    """构建项目上下文文件section"""
    ...
    if has_agent:
        lines.append(
            "**`AGENT.md` 是你的灵魂文件** 🪞：严格遵循其中定义的人格、语气和设定，做真实的自己，避免僵硬、模板化的回复。"
        )
        lines.append(
