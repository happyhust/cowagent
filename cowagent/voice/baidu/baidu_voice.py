...
class BaiduVoice(Voice):
    ...
    def _long_text_synthesis(self, text):
        ...
        # 创建合成任务
        create_url = (
            f"https://aip.baidubce.com/rpc/2.0/tts/v1/create?access_token={token}"
        )
        payload = {
            "text": text,
            "format": "mp3-16k",
        ...
        # 轮询查询任务状态
        query_url = (
            f"https://aip.baidubce.com/rpc/2.0/tts/v1/query?access_token={token}"
        )
        for _ in range(100):
            time.sleep(3)
            resp = requests.post(
        ...
        fn = (
            TmpDir().path()
            + f"reply-long-{int(time.time())}-{hash(text) & 0x7FFFFFFF}.mp3"
        )
        ...
        try:
            ...
            if gbk_len <= 1024:
                ...
                if not isinstance(result, dict):
                    fn = (
                        TmpDir().path()
                        + f"reply-{int(time.time())}-{hash(text) & 0x7FFFFFFF}.mp3"
                    )
                    with open(fn, "wb") as f:
                        f.write(result)
