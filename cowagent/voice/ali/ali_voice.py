...
from cowagent.voice.ali.ali_api import (
...
try:
    from cowagent.voice.audio_convert import get_pcm_from_wav
except ImportError as e:
    logger.debug("import cowagent.voice.audio_convert failed: {}".format(e))
...
class AliVoice(Voice):
    def __init__(self):
        ...
        try:
            ...
            self.access_key_id = conf().get("qwen_access_key_id") or config.get(
                "access_key_id"
            )
            self.access_key_secret = conf().get("qwen_access_key_secret") or config.get(
                "access_key_secret"
            )
    ...
    def textToVoice(self, text):
        ...
        text = re.sub(
            r"[^\u4e00-\u9fa5\u3040-\u30FF\uAC00-\uD7AFa-zA-Z0-9"
            r"äöüÄÖÜáéíóúÁÉÍÓÚàèìòùÀÈÌÒÙâêîôûÂÊÎÔÛçÇñÑ，。！？,.]",
            "",
            text,
        )
        # 提取有效的token
        token_id = self.get_valid_token()
        fileName = text_to_speech_aliyun(
            self.api_url_text_to_voice, text, self.app_key, token_id
        )
        if fileName:
            logger.info(
                "[Ali] textToVoice text={} voice file name={}".format(text, fileName)
            )
    ...
    def voiceToText(self, voice_file):
        ...
        text = speech_to_text_aliyun(
            self.api_url_voice_to_text, pcm, self.app_key, token_id
        )
        if text:
