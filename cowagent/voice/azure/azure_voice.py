...
class AzureVoice(Voice):
    def __init__(self):
        try:
            ...
            self.speech_config.speech_synthesis_voice_name = self.config[
                "speech_synthesis_voice_name"
            ]
            self.speech_config.speech_recognition_language = self.config[
    ...
    def voiceToText(self, voice_file):
        ...
        if result.reason == speechsdk.ResultReason.RecognizedSpeech:
            ...
            logger.error(
                "[Azure] voiceToText error, result={}, errordetails={}".format(
    ...
    def textToVoice(self, text):
        if self.config.get("auto_detect"):
            ...
            if key in self.config:
                logger.info(
                    "[Azure] textToVoice auto detect language={}, voice={}".format(
                        lang, self.config[key]
                    )
                )
        ...
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            ...
            cancel_details = result.cancellation_details
            logger.error(
                "[Azure] textToVoice error, result={}, errordetails={}".format(
                    result, cancel_details.error_details
                )
            )
