...
from cowagent.agent.protocol import Agent
from cowagent.agent.tools import ToolManager
...
class AgentInitializer:
    ...
    def initialize_agent(self, session_id: Optional[str] = None) -> Agent:
        ...
        if session_id is None:
            logger.info(
                f"[AgentInitializer] Workspace initialized at: {workspace_root}"
            )
        ...
        tools = self._load_tools(
            workspace_root, memory_manager, memory_tools, session_id
        )
    ...
    def _setup_memory_system(
        self, workspace_root: str, session_id: Optional[str] = None
    ):
        """
        Setup memory system
        ...
        try:
            from cowagent.agent.memory import (
                MemoryManager,
                MemoryConfig,
                create_embedding_provider,
            )
            from cowagent.agent.tools import MemorySearchTool, MemoryGetTool
            from cowagent.config import conf
            ...
            if embedding_provider is None:
                ...
                if linkai_api_key and linkai_api_key not in [
                    ...
                    try:
                        ...
                        if session_id is None:
                            logger.info(
                                "[AgentInitializer] LinkAI embedding initialized (fallback)"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[AgentInitializer] LinkAI embedding failed: {e}"
                        )

            # Create memory manager
            memory_config = MemoryConfig(workspace_root=workspace_root)
            memory_manager = MemoryManager(
                memory_config, embedding_provider=embedding_provider
            )
    ...
    def _load_tools(
        ...
        tools = []
        file_config = (
            {"cwd": workspace_root, "memory_manager": memory_manager}
            if memory_manager
            else {"cwd": workspace_root}
        )
        ...
        for tool_name in tool_manager.tool_classes.keys():
            try:
                ...
                if tool_name == "web_search":
                    from cowagent.agent.tools.web_search.web_search import WebSearch

                    if not WebSearch.is_available():
                        logger.debug(
                            "[AgentInitializer] WebSearch skipped - no BOCHA_API_KEY or LINKAI_API_KEY"
                        )
                        continue

                # Special handling for EnvConfig tool
                if tool_name == "env_config":
                    from cowagent.agent.tools import EnvConfig
                ...
                logger.warning(
                    f"[AgentInitializer] Failed to load tool {tool_name}: {e}"
                )
        ...
        if memory_tools:
            ...
            if session_id is None:
                logger.info(
                    f"[AgentInitializer] Added {len(memory_tools)} memory tools"
                )

        if session_id is None:
            logger.info(
                f"[AgentInitializer] Loaded {len(tools)} tools: {[t.name for t in tools]}"
            )
    ...
    def _initialize_scheduler(self, tools: List, session_id: Optional[str] = None):
        ...
        if not self.agent_bridge.scheduler_initialized:
            try:
                from cowagent.agent.tools.scheduler.integration import init_scheduler
                ...
                logger.warning(
                    f"[AgentInitializer] Failed to initialize scheduler: {e}"
                )

        # Inject scheduler dependencies
        if self.agent_bridge.scheduler_initialized:
            try:
                from cowagent.agent.tools.scheduler.integration import (
                    get_task_store,
                    get_scheduler_service,
                )
                from cowagent.agent.tools import SchedulerTool
                from cowagent.config import conf
                ...
                logger.warning(
                    f"[AgentInitializer] Failed to inject scheduler dependencies: {e}"
                )
    ...
    def _initialize_skill_manager(
        ...
        """Initialize skill manager"""
        try:
            from cowagent.agent.skills import SkillManager
    ...
    def _get_runtime_info(self, workspace_root: str):
        ...
        def get_current_time():
            ...
            try:
                ...
                timezone_name = (
                    f"UTC{hours:+03d}:{minutes:02d}" if minutes else f"UTC{hours:+03d}"
                )
            except Exception:
                timezone_name = "UTC"
        ...
        return {
            ...
            "channel": ", ".join(conf().get("channel_type"))
            if isinstance(conf().get("channel_type"), list)
            else conf().get("channel_type", "unknown"),
    ...
    def _migrate_config_to_env(self, workspace_root: str):
        ...
        if keys_to_migrate:
            try:
                ...
                logger.info(
                    f"[AgentInitializer] Migrated {len(keys_to_migrate)} API keys to .env: {list(keys_to_migrate.keys())}"
                )
            except Exception as e:
        ...
        def _daily_flush_loop():
            while True:
                try:
                    ...
                    logger.info(
                        f"[DailyFlush] Next flush at {target.strftime('%Y-%m-%d %H:%M')} (in {wait_seconds / 3600:.1f}h)"
                    )
    ...
    def _flush_all_agents(self):
        ...
        if flushed:
            logger.info(
                f"[DailyFlush] Flushed {flushed}/{len(agents)} agent session(s)"
            )
