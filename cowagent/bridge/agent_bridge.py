...
from cowagent.agent.protocol import Agent, LLMModel, LLMRequest
from cowagent.bridge.agent_event_handler import AgentEventHandler
from cowagent.bridge.agent_initializer import AgentInitializer
...
def add_openai_compatible_support(bot_instance):
    ...
    class EnhancedBot(bot_instance.__class__, OpenAICompatibleBot):
        ...
        def get_api_config(self):
            """
            Infer API config from cowagent.common configuration patterns.
...
class AgentBridge:
                ...
                if new_messages:
                    ...
                    if msg_count == 0:
                        try:
                            ...
                            logger.info(
                                f"[AgentBridge] Cleared DB for recovered session: {session_id}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"[AgentBridge] Failed to clear DB after recovery: {e}"
                            )
            ...
            if session_id and agent:
                try:
                    ...
                    if msg_count == 0:
                        ...
                        get_conversation_store().clear_session(session_id)
                        logger.info(
                            f"[AgentBridge] Cleared DB for session after error: {session_id}"
                        )
                except Exception as db_err:
                    logger.warning(
                        f"[AgentBridge] Failed to clear DB after error: {db_err}"
                    )
    ...
    def _create_file_reply(
        ...
        Args:
            ...
            text_response: Text response from cowagent.agent
    ...
    def _migrate_config_to_env(self, workspace_root: str):
        ...
        if existing_env_vars:
            logger.debug(
                f"[AgentBridge] {len(existing_env_vars)} env vars already in .env"
            )
        ...
        if keys_to_migrate:
            try:
                ...
                logger.info(
                    f"[AgentBridge] Migrated {len(keys_to_migrate)} API keys from config.json to .env: {list(keys_to_migrate.keys())}"
                )
            except Exception as e:
