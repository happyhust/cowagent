...
from cowagent.config import conf, load_config, global_config
...
def get_help_text(isadmin, isgroup):
    ...
    for cmd, info in COMMANDS.items():
        ...
        active_channels = (
            ...
            else [c.strip() for c in str(raw_ct).split(",")]
        )
...
class Godcmd(Plugin):
    def __init__(self):
        ...
        if gconf["password"] == "":
            ...
            logger.info(
                "[Godcmd] 因未设置口令，本次的临时口令为%s。" % self.temp_password
            )
        else:
                ...
                if cmd == "auth":
                    ...
                    if bottype in [
                        ...
                        if Bridge().chat_bots.get(bottype):
                            Bridge().chat_bots.get(bottype).sessions.clear_session(
                                session_id
                            )
    ...
    def is_admin_in_group(self, context):
        if context["isgroup"]:
            return (
                context.kwargs.get("msg").actual_user_id in global_config["admin_users"]
            )
        return False
