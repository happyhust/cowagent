...
class Hello(Plugin):
    group_welc_prompt = (
        '请你随机使用一种风格说一句问候语来欢迎新用户"{nickname}"加入群聊。'
    )
    group_exit_prompt = (
        "请你随机使用一种风格介绍你自己，并告诉用户输入#help可以查看帮助信息。"
    )
    ...
    def __init__(self):
        ...
        try:
            ...
            self.group_welc_prompt = self.config.get(
                "group_welc_prompt", self.group_welc_prompt
            )
            self.group_exit_prompt = self.config.get(
                "group_exit_prompt", self.group_exit_prompt
            )
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if e_context["context"].type == ContextType.JOIN_GROUP:
            ...
            e_context["context"].content = self.group_welc_prompt.format(
        ...
        if e_context["context"].type == ContextType.EXIT_GROUP:
            if conf().get("group_chat_exit_group"):
                e_context["context"].type = ContextType.TEXT
                e_context["context"].content = self.group_exit_prompt.format(
                    nickname=msg.actual_user_nickname
                )
                e_context.action = EventAction.BREAK  # 事件结束，进入默认处理逻辑
                return
    ...
    def _load_config_template(self):
        logger.debug(
            "No Hello plugin config.json, use plugins/hello/config.json.template"
        )
        try:
