...
class CowCliPlugin(Plugin):
    ...
    def _dispatch(
        self, cmd: str, args: str, e_context: EventContext, session_id: str = ""
    ) -> str:
    ...
    def _cmd_status(
        self, args: str, e_context: EventContext, session_id: str = ""
    ) -> str:
        ...
        if agent:
            ...
            if agent.skill_manager:
                ...
                enabled = sum(
                    1
                    for v in agent.skill_manager.skills_config.values()
    ...
    def _find_log_file(self) -> str:
        ...
        candidates.extend(
            sorted(
                glob_mod.glob(os.path.join(project_root, "logs", "*.log")), reverse=True
            )
        )
        for f in candidates:
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        from cowagent.cli.commands.install import run_install_browser
                ...
                for name in sorted(os.listdir(d)):
                    ...
                    if os.path.isdir(skill_path) and not name.startswith("."):
                        if os.path.exists(os.path.join(skill_path, "SKILL.md")):
                            entries.append(
                                {"name": name, "source": source, "enabled": True}
                            )
            if not entries:
                return "暂无已安装的技能\n\n💡 /skill list --remote 浏览技能广场"
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        import requests
        from cowagent.cli.utils import SKILL_HUB_API, load_skills_config
    ...
    def _skill_install(self, name: str, e_context: EventContext) -> str:
        ...
        from cowagent.cli.commands.skill import install_skill
    ...
    def _format_install_result(result) -> str:
        ...
        from cowagent.cli.commands.skill import _read_skill_description
        from cowagent.cli.utils import get_skills_dir, load_skills_config
    ...
    def _skill_uninstall(self, name: str) -> str:
        ...
        import shutil
        import json
        from cowagent.cli.utils import get_skills_dir
    ...
    def _skill_info(self, name: str) -> str:
        ...
        from cowagent.cli.utils import get_skills_dir, get_builtin_skills_dir
