...
class Keyword(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if content in self.keyword:
            ...
            # 判断匹配内容的类型
            if (
                reply_text.startswith("http://") or reply_text.startswith("https://")
            ) and any(
                reply_text.endswith(ext)
                for ext in [".jpg", ".webp", ".jpeg", ".png", ".gif", ".img"]
            ):
                ...
                reply_text.startswith("http://") or reply_text.startswith("https://")
            ) and any(reply_text.endswith(ext) for ext in [".mp4"]):
