...
class LinkSummary:
    ...
    def summary_file(self, file_path: str, app_code: str):
        ...
        res = requests.post(
            url, headers=self.headers(), files=file_body, data=body, timeout=(5, 300)
        )
    ...
    def summary_url(self, url: str, app_code: str):
        ...
        res = requests.post(
            url=self.base_url() + "/v1/summary/url",
    ...
    def summary_chat(self, summary_id: str):
        ...
        res = requests.post(
            url=self.base_url() + "/v1/summary/chat",
            headers=self.headers(),
            json=body,
            timeout=(5, 180),
        )
        if res.status_code == 200:
            ...
            res_json = res.json()
            logger.error(
                f"[LinkSum] summary error, status_code={res.status_code}, msg={res_json.get('message')}"
            )
            return None
    ...
    def check_file(self, file_path: str, sum_config: dict) -> bool:
        ...
        if (
            sum_config.get("max_file_size")
            and file_size > sum_config.get("max_file_size")
        ) or file_size > 15000:
            logger.warn(
                f"[LinkSum] file size exceeds limit, No processing, file_size={file_size}KB"
            )
            return False
        ...
        if suffix not in support_list:
            logger.warn(
                f"[LinkSum] unsupported file, suffix={suffix}, support_list={support_list}"
            )
            return False
