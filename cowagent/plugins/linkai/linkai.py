...
from cowagent.bridge import bridge
...
class LinkAI(Plugin):
    def __init__(self):
        ...
        if self.config:
            self.mj_bot = MJBot(
                self.config.get("midjourney"), self._fetch_group_app_code
            )
        self.sum_config = {}
        if self.config:
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if context.type in [
            ContextType.FILE,
            ContextType.IMAGE,
        ] and self._is_summary_open(context):
            # 文件处理
            ...
            if context.type != ContextType.IMAGE:
                USER_FILE_MAP[_find_user_id(context) + "-sum_id"] = res.get(
                    "summary_id"
                )
        ...
        if (context.type == ContextType.SHARING and self._is_summary_open(context)) or (
            ...
            and self._is_summary_open(context)
            and LinkSummary().check_url(context.content)
        ):
        ...
        if (
            context.type == ContextType.TEXT
            and context.content == "开启对话"
            and _find_sum_id(context)
        ):
            # 文本对话
        ...
        if (
            context.type == ContextType.TEXT
            and context.content == "退出对话"
            and _find_file_id(context)
        ):
    ...
    def _process_admin_cmd(self, e_context: EventContext):
        ...
        if len(cmd) == 2 and (cmd[1] == "open" or cmd[1] == "close"):
            ...
            _set_reply_text(
                f"LinkAI对话功能{tips_text}", e_context, level=ReplyType.INFO
            )
            return
        ...
        if len(cmd) == 3 and cmd[1] == "app":
            ...
            if not context.kwargs.get("isgroup"):
                _set_reply_text(
                    "该指令需在群聊中使用", e_context, level=ReplyType.ERROR
                )
                return
            if not Util.is_admin(e_context):
                _set_reply_text("需要管理员权限执行", e_context, level=ReplyType.ERROR)
            ...
            _set_reply_text(
                f"应用设置成功: {app_code}", e_context, level=ReplyType.INFO
            )
            return
        ...
        if (
            ...
            if not self.sum_config:
                ...
                _set_reply_text(
                    f"文章总结功能{tips_text}", e_context, level=ReplyType.INFO
                )
            return

        _set_reply_text(
            f"指令错误，请输入{_get_trigger_prefix()}linkai help 获取帮助",
            e_context,
            level=ReplyType.INFO,
        )
        return
    ...
    def _is_summary_open(self, context) -> bool:
        ...
        base_enabled = (
            ...
            and (
                context.type.name
                in (self.sum_config.get("type") or ["FILE", "SHARING"])
                or context.type.name == "TEXT"
            )
        )
        ...
        if context.kwargs.get("isgroup"):
            return (
                base_enabled and self.sum_config.get("group_enabled")
            ) or remote_enabled
    ...
    def _load_config_template(self):
        logger.debug(
            "No LinkAI plugin config.json, use plugins/linkai/config.json.template"
        )
        try:
