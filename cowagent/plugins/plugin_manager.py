...
class PluginManager:
        ...
        try:
            if os.path.exists(all_config_path):
                ...
                with open(all_config_path, "r", encoding="utf-8") as f:
                    ...
                    logger.info(
                        f"load all config from cowagent.plugins/config.json: {all_conf}"
                    )
            ...
            for name in self.listening_plugins[e_context.event]:
                if (
                    ...
                    if e_context.is_break():
                        ...
                        logger.debug(
                            "Plugin %s breaked event %s" % (name, e_context.event)
                        )
        return e_context
    ...
    def install_plugin(self, repo: str):
        try:
            import cowagent.common.package_manager as pkgmgr
