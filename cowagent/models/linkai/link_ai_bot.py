...
import threading
from cowagent.common import memory, utils
import base64
import os
...
class LinkAIBot(Bot, OpenAICompatibleBot):
    ...
    def reply(self, query, context: Context = None) -> Reply:
        if context.type == ContextType.TEXT:
            ...
            if not conf().get("text_to_image"):
                logger.warn(
                    "[LinkAI] text_to_image is not enabled, ignore the IMAGE_CREATE request"
                )
    ...
    def _chat(self, query, context, retry_count=0) -> Reply:
        ...
        try:
            ...
            if img_cache:
                messages = self._process_image_msg(
                    app_code=app_code,
                ...
                if client_id:
                    ...
                    if context.kwargs.get("msg"):
                        ...
                        if context.kwargs.get("msg").is_group:
                            body["is_group"] = True
                            body["group_name"] = context.kwargs.get(
                                "msg"
                            ).from_user_nickname
                            body["sender_name"] = context.kwargs.get(
                                "msg"
            ...
            logger.info(
                f"[LINKAI] query={query}, app_code={app_code}, model={body.get('model')}, file_id={file_id}"
            )
            ...
            if res.status_code == 200:
                ...
                logger.info(
                    f"[LINKAI] reply={reply_content}, total_tokens={total_tokens}, res_code={res_code}"
                )
                if res_code == 429:
                    logger.warn(
                        f"[LINKAI] 用户访问超出限流配置，sender_id={body.get('sender_id')}"
                    )
                else:
                    self.sessions.session_reply(
                        reply_content, session_id, total_tokens, query=query
                    )
                ...
                if response["choices"][0].get("img_urls"):
                    thread = threading.Thread(
                        ...
                        args=(
                            context.get("channel"),
                            context,
                            response["choices"][0].get("img_urls"),
                        ),
                    )
                    thread.start()
                ...
                logger.error(
                    f"[LINKAI] chat failed, status_code={res.status_code}, "
    ...
    def _process_image_msg(
        self, app_code: str, session_id: str, query: str, img_cache: dict
    ):
        try:
            ...
            if not app_info:
                logger.debug(
                    f"[LinkAI] not found app, can't process images, app_code={app_code}"
                )
                return None
    ...
    def reply_text(self, session: ChatGPTSession, app_code="", retry_count=0) -> dict:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                total_tokens = response["usage"]["total_tokens"]
                logger.info(
                    f"[LINKAI] reply={reply_content}, total_tokens={total_tokens}"
                )
                return {
    ...
    def _fetch_knowledge_search_suffix(self, response) -> str:
        try:
            if response.get("knowledge_base"):
                ...
                first_similarity = response.get("knowledge_base").get(
                    "first_similarity"
                )
                logger.info(
                    f"[LINKAI] knowledge base, search_hit={search_hit}, first_similarity={first_similarity}"
                )
                plugin_config = pconf("linkai")
                if (
                    plugin_config
                    and plugin_config.get("knowledge_base")
                    and plugin_config.get("knowledge_base").get(
                        "search_miss_text_enabled"
                    )
                ):
                    search_miss_similarity = plugin_config.get("knowledge_base").get(
                        "search_miss_similarity"
                    )
                    search_miss_text = plugin_config.get("knowledge_base").get(
                        "search_miss_suffix"
                    )
                    if not search_hit:
                    ...
                    if (
                        search_miss_similarity
                        and float(search_miss_similarity) > first_similarity
                    ):
                        return search_miss_text
        except Exception as e:
            logger.exception(e)
    ...
    def _fetch_agent_suffix(self, response):
        try:
            ...
            if (
                response.get("agent")
                and response.get("agent").get("chain")
                and response.get("agent").get("need_show_plugin")
            ):
...
class LinkAISessionManager(SessionManager):
    ...
    def session_reply(self, reply, session_id, total_tokens=None, query=None):
        ...
        try:
            ...
            logger.debug(
                f"[LinkAI] chat history, before tokens={total_tokens}, now tokens={tokens_cnt}"
            )
        except Exception as e:
            logger.warning(
    ...
    def discard_exceeding(self, max_tokens, cur_tokens=None):
        ...
        if cur_tokens > max_tokens:
            for i in range(0, len(self.messages)):
                if (
                    i > 0
                    and self.messages[i].get("role") == "assistant"
                    and self.messages[i - 1].get("role") == "user"
                ):
                    self.messages.pop(i)
...
def _linkai_call_with_tools(self, messages, tools=None, stream=False, **kwargs):
    ...
    try:
        ...
        logger.debug(
            f"[LinkAI] messages: {len(messages)}, tools: {len(tools) if tools else 0}, stream: {stream}"
        )
...
def _handle_linkai_sync_response(self, base_url, headers, body):
    ...
    try:
        ...
        if res.status_code == 200:
            response = res.json()
            logger.debug(
                f"[LinkAI] reply: model={response.get('model')}, "
...
def _handle_linkai_stream_response(self, base_url, headers, body):
    ...
    try:
        ...
        if res.status_code != 200:
            ...
            yield {"error": True, "status_code": res.status_code, "message": error_msg}
            return
            ...
            if line:
                ...
                if line.startswith("data: "):
                    ...
                    if chunk.get("type") == "error" or (
                        isinstance(chunk.get("error"), dict)
                        and "message" in chunk.get("error", {})
                    ):
                        ...
                        http_code = (
                            error_data.get("http_code", "")
                            if isinstance(error_data, dict)
                            else ""
                        )
                        status_code = (
                            int(http_code)
                            if http_code and str(http_code).isdigit()
                            else 400
                        )
                        logger.error(
                            f"[LinkAI] stream error: {error_msg} (http_code={http_code})"
                        )
                        yield {
                            "error": True,
                            "message": error_msg,
