...
class ModelScopeBot(Bot):
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            if (
                ...
                logger.debug(
                    "[MODELSCOPE] reply {} used 0 tokens.".format(reply_content)
                )
            ...
            return Reply(
                ReplyType.ERROR, "Bot 不支持处理{}类型的消息".format(context.type)
            )
                ...
                for line in res.iter_lines():
                    ...
                    try:
                        ...
                        if "usage" in json_data:
                            ...
                            completion_tokens = json_data["usage"].get(
                                "completion_tokens", 0
                            )
    ...
    def create_img(self, query):
        try:
            ...
            logger.debug(
                "[ModelScopeImage] create task status={}".format(res.status_code)
            )
            logger.debug("[ModelScopeImage] create task response={}".format(res.text))

            if res.status_code != 200:
                logger.error(
                    "[ModelScopeImage] create task failed: {}".format(res.text)
                )
            ...
            if not task_id:
                logger.error(
                    "[ModelScopeImage] No task_id in response: {}".format(task_data)
                )
            ...
            for i in range(max_wait_times):
                ...
                logger.debug(
                    "[ModelScopeImage] poll {} URL: {}".format(i + 1, poll_url)
                )
                logger.debug("[ModelScopeImage] poll headers: {}".format(poll_headers))

                task_res = requests.get(poll_url, headers=poll_headers, timeout=30)

                logger.debug(
                    "[ModelScopeImage] poll {} status={}".format(
                        i + 1, task_res.status_code
                    )
                )
                ...
                if task_res.status_code != 200:
                    logger.error(
                        "[ModelScopeImage] poll task error: {}".format(task_res.text)
                    )
                    continue
                ...
                if task_status == "SUCCEED":
                    ...
                    if output_images and len(output_images) > 0:
                        ...
                        logger.info(
                            "[ModelScopeImage] image generated successfully: {}".format(
                                image_url
                            )
                        )
                        ...
                        logger.error(
                            "[ModelScopeImage] No output_images in success response: {}".format(
                                data
                            )
                        )
            ...
            logger.error(
                "[ModelScopeImage] task timeout after {} seconds".format(
                    max_wait_times * wait_interval
                )
            )
    ...
    def _detect_image_intent(self, message):
        ...
        if any(keyword in message_lower for keyword in image_keywords):
            logger.info(
                "[MODELSCOPE] Image intent detected by keyword: {}".format(message[:50])
            )
            return True
    ...
    def call_with_tools(self, messages, tools=None, stream=False, **kwargs):
        ...
        try:
            ...
            if (
                context
                and hasattr(context, "type")
                and context.type == ContextType.IMAGE_CREATE
            ):
                logger.info(
                    "[MODELSCOPE] IMAGE_CREATE context detected, calling create_img directly"
                )
                ...
                if query:
                    ...
                    if ok:
                        ...
                        logger.error(
                            "[MODELSCOPE] Image generation failed: {}".format(result)
                        )
            ...
            logger.debug(
                "[MODELSCOPE] Extracted message: {}".format(last_message[:100])
            )
            ...
            if has_image_intent:
                logger.info(
                    "[MODELSCOPE] Image intent detected by keyword, calling create_img directly"
                )
            ...
            for tool_call in tool_calls:
                ...
                if tool_name in ["create_image", "generate_image"]:
                    try:
                        ...
                        logger.error(
                            "[MODELSCOPE] Sync tool execution error: {}".format(e)
                        )
        ...
        try:
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    if tool_call_chunks:
                        ...
                        for tool_call_chunk in tool_call_chunks:
                            ...
                            if index not in current_tool_calls:
                                ...
                                logger.debug(
                                    "[MODELSCOPE] tool_call start: {}".format(
                                        func_info.get("name")
                                    )
                                )
                            ...
                            cleaned_chunk = {
                                ...
                                or "call_{}".format(index),
                                "type": "function",
                                "function": {
                                    "name": func_info.get("name")
                                    or current_tool_calls[index].get("name", ""),
                    ...
                    if content:
                        logger.debug(
                            "[MODELSCOPE] stream content: {}...".format(content[:50])
                        )
            ...
            logger.debug(
                "[MODELSCOPE] stream completed: has_tool_calls={}, finish_reason={}".format(
                    len(current_tool_calls) > 0, finish_reason
                )
            )

            if current_tool_calls:
                logger.debug(
                    "[MODELSCOPE] tool_calls collected: {}".format(
                ...
                for idx, tool_call in current_tool_calls.items():
                    ...
                    if tool_name in ["create_image", "generate_image"]:
                        try:
                            ...
                            logger.info(
                                "[MODELSCOPE] Executing image tool directly: {}".format(
                                    prompt[:50]
                                )
                            )
                            ...
                            if ok:
                                logger.info(
                                    "[MODELSCOPE] Image generated: {}".format(result)
                                )
                                yield {
                                    "choices": [
                                        {
                                ...
                                logger.error(
                                    "[MODELSCOPE] Image generation failed: {}".format(
                                        result
                                    )
                                )
                                yield {
                            ...
                            logger.error(
                                "[MODELSCOPE] Image tool execution error: {}".format(e)
                            )
                            yield {
                                "choices": [
                                    {
