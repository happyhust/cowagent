...
class MoonshotBot(Bot):
    ...
    def reply_text(
        self, session: MoonshotSession, args=None, retry_count: int = 0
    ) -> dict:
        """
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    f"[MOONSHOT] chat failed, status_code={res.status_code}, "
            ...
            logger.error(
                f"[MOONSHOT] reply_text error | model={self.args.get('model')} | "
    ...
    def call_with_tools(self, messages, tools=None, stream: bool = False, **kwargs):
        ...
        try:
            ...
            if system_prompt:
                if (
                    not converted_messages
                    or converted_messages[0].get("role") != "system"
                ):
            ...
            logger.debug(
                f"[MOONSHOT] API call: model={model}, "
            ...
            error_msg = str(e)
            model_name = kwargs.get("model", self.args.get("model", "moonshot-v1-8k"))
            msg_count = len(messages) if messages else 0
            tool_count = len(tools) if tools else 0
            logger.error(
                f"[MOONSHOT] call_with_tools error | model={model_name} | "
    ...
    def _handle_stream_response(self, request_body: dict):
        ...
        try:
            ...
            response = requests.post(
                url, headers=headers, json=request_body, stream=True, timeout=120
            )

            if response.status_code != 200:
                error_msg = response.text
                logger.error(
                    f"[MOONSHOT] API error: status={response.status_code}, msg={error_msg}"
                )
                yield {
                    "error": True,
                    "message": error_msg,
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    logger.warning(
                        f"[MOONSHOT] JSON decode error: {e}, data: {data_str[:200]}"
                    )
                    continue
    ...
    def _handle_sync_response(self, request_body: dict):
        ...
        try:
            ...
            url = f"{self.base_url}/chat/completions"
            response = requests.post(
                url, headers=headers, json=request_body, timeout=120
            )
