...
from cowagent.models.minimax.minimax_session import MinimaxSession
...
class MinimaxBot(Bot):
    def __init__(self):
        super().__init__()
        self.args = {
            "model": conf().get("llm_model") or "MiniMax-M2.1",
    ...
    @property
    def api_base(self):
        return conf().get("llm_api_base", "https://api.minimaxi.com/v1")
    ...
    def reply_text(self, session: MinimaxSession, args=None, retry_count=0) -> dict:
        ...
        try:
            ...
            response = requests.post(
                url, headers=headers, json=request_body, timeout=60
            )
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"[MINIMAX] reply_text: content_length={len(content)}, tokens={total_tokens}"
                )
                ...
                logger.error(
                    f"[MINIMAX] API error: status={response.status_code}, msg={error_msg}"
                )
    ...
    def call_with_tools(self, messages, tools=None, stream=False, **kwargs):
        ...
        Args:
            messages: List of messages (may be in Claude format from cowagent.agent)
            tools: List of tool definitions (may be in Claude format from cowagent.agent)
        ...
        try:
            ...
            logger.debug(
                f"[MINIMAX] API call: model={model}, tools={len(converted_tools) if converted_tools else 0}, stream={stream}"
            )

            # Check if we should show thinking process
            show_thinking = kwargs.pop(
                "show_thinking", conf().get("minimax_show_thinking", False)
            )

            if stream:
                return self._handle_stream_response(
                    request_body, show_thinking=show_thinking
                )
            else:
    ...
    def _handle_stream_response(self, request_body, show_thinking=False):
        ...
        try:
            ...
            url = f"{self.api_base}/chat/completions"
            response = requests.post(
                url, headers=headers, json=request_body, stream=True, timeout=60
            )
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    logger.warning(
                        f"[MINIMAX] JSON decode error: {e}, data: {data_str[:100]}"
                    )
                    continue
                ...
                if chunk.get("type") == "error" or "error" in chunk:
                    ...
                    logger.error(
                        f"[MINIMAX] API error: {error_msg} (type: {error_type}, code: {http_code})"
                    )
                ...
                if "content" in delta and delta["content"]:
                    ...
                    if not any(
                        block.get("type") == "text" for block in current_content
                    ):
                    ...
                    for tool_call_chunk in delta["tool_calls"]:
                        ...
                        if index not in current_tool_calls:
                            ...
                            current_tool_calls[index] = {
                                ...
                                "type": "tool_use",
                                "name": tool_call_chunk.get("function", {}).get(
                                    "name", ""
                                ),
                                "input": "",
                            }
                        ...
                        if (
                            "function" in tool_call_chunk
                            and "arguments" in tool_call_chunk["function"]
                        ):
                            current_tool_calls[index]["input"] += tool_call_chunk[
                                "function"
                            ]["arguments"]
                        ...
                        yield {
                            "choices": [
                                {"index": 0, "delta": {"tool_calls": [tool_call_chunk]}}
                            ]
                        }
