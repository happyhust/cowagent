...
class GoogleGeminiBot(Bot):
    ...
    def reply(self, query, context: Context = None) -> Reply:
        ...
        try:
            ...
            response = self.call_with_tools(
                messages=filtered_messages, tools=None, stream=False, model=self.model
            )

            if isinstance(response, dict) and response.get("error"):
                error_message = response.get(
                    "message", "Failed to invoke [Gemini] api!"
                )
            ...
            logger.warning(
                "[Gemini] No valid response generated. Checking safety ratings."
            )
            safety_ratings = (
                response.get("safety_ratings", []) if isinstance(response, dict) else []
            )
            if safety_ratings:
                for rating in safety_ratings:
                    ...
                    logger.warning(
                        f"[Gemini] Safety rating: {category} - {probability}"
                    )
    ...
    def _extract_image_paths_from_text(content: str):
        ...
        pattern = r"\[图片:\s*([^\]]+)\]"
        image_paths = [
            m.strip().strip("'\"") for m in re.findall(pattern, content) if m.strip()
        ]
    ...
    def _build_image_inline_part(image_path: str):
        ...
        try:
            ...
            logger.warning(
                f"[Gemini] Failed to build inline image part from path={image_path}, err={e}"
            )
            return None
    ...
    def _build_inline_part_from_image_url(image_url):
        ...
        if image_url.startswith("file://") or os.path.exists(
        ...
        if image_url.startswith("http://") or image_url.startswith("https://"):
            try:
                ...
                if response.status_code != 200:
                    logger.warning(
                        f"[Gemini] Failed to fetch remote image: status={response.status_code}, url={image_url}"
                    )
                    return None
                mime_type = (
                ...
                logger.warning(
                    f"[Gemini] Failed to download remote image: url={image_url}, err={e}"
                )
                return None
        ...
        try:
            ...
            for msg in messages:
                ...
                if isinstance(content, str):
                    ...
                    cleaned_text, image_paths = self._extract_image_paths_from_text(
                        content
                    )
                    if cleaned_text:
                    ...
                    for block in content:
                        ...
                        if block_type == "text":
                            ...
                            cleaned_text, image_paths = (
                                self._extract_image_paths_from_text(block_text)
                            )
                            if cleaned_text:
                                parts.append({"text": cleaned_text})
                            for image_path in image_paths:
                            ...
                            if block_type == "image":
                                ...
                                if (
                                    ...
                                    image_part = self._build_inline_part_from_image_url(
                                        block.get("image_url")
                                    )
                            else:
                                image_part = self._build_inline_part_from_image_url(
                            ...
                            if image_part:
                                ...
                                logger.warning(
                                    f"[Gemini] Skip invalid image block: {str(block)[:200]}"
                                )
                ...
                if parts:
                    payload["contents"].append({"role": gemini_role, "parts": parts})

            if inline_image_count > 0:
                logger.info(
                    f"[Gemini] Multimodal request includes {inline_image_count} image part(s)"
                )
            ...
            if stream:
                endpoint = (
                    f"{base_url}/models/{model_name}:streamGenerateContent?alt=sse"
                )
            ...
            response = requests.post(
                endpoint, headers=headers, json=payload, stream=stream, timeout=60
            )
    ...
    def _handle_gemini_rest_sync_response(self, response, model_name):
        ...
        try:
            ...
            data = response.json()
            logger.debug(
                f"[Gemini] Response data: {json.dumps(data, ensure_ascii=False)[:500]}"
            )
            ...
            logger.info(
                f"[Gemini] Response: text={len(text_content)} chars, tool_calls={len(tool_calls)}"
            )

            # Build OpenAI format response
            message_dict = {"role": "assistant", "content": text_content or None}
            if tool_calls:
                            ...
                            all_tool_calls.append(
                                {
                                    "index": len(
                                        all_tool_calls
                                    ),  # Add index to differentiate multiple tool calls
    ...
    def _handle_gemini_stream_response(
        self, model, messages, request_params, model_name
    ):
        ...
        try:
            response_stream = model.generate_content(
                messages, stream=True, **request_params
            )
