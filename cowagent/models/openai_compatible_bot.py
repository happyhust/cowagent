        ...
        try:
            ...
            request_params = {
                ...
                "temperature": kwargs.get(
                    "temperature", api_config.get("default_temperature", 0.9)
                ),
                "top_p": kwargs.get("top_p", api_config.get("default_top_p", 1.0)),
                "frequency_penalty": kwargs.get(
                    "frequency_penalty",
                    api_config.get("default_frequency_penalty", 0.0),
                ),
                "presence_penalty": kwargs.get(
                    "presence_penalty", api_config.get("default_presence_penalty", 0.0)
                ),
                "stream": stream,
            }
                ...
                # Check if this is an assistant message with tool_use blocks
                    ...
                    for block in content:
                        if block.get("type") == "text":
                            ...
                            if not tool_id:
                                logger.warning(
                                    f"[OpenAICompatible] tool_use missing id for '{block.get('name')}'"
                                )
                            tool_calls.append(
                                {
                                    "id": tool_id,
