    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            if (
                ...
                reply = Reply(ReplyType.ERROR, reply_content["content"])
                logger.debug(
                    "[DASHSCOPE] reply {} used 0 tokens.".format(reply_content)
                )
            return reply
        else:
            reply = Reply(
        ...
        try:
            ...
            if self._is_multimodal_model(self.model_name):
                ...
                response = MultiModalConversation.call(
                    model=model, messages=mm_messages, result_format="message"
                )
            else:
                response = self.client.call(
                    model, messages=session.messages, result_format="message"
                )
        ...
        try:
            ...
            parameters = {
                ...
                "temperature": kwargs.get(
                    "temperature", conf().get("temperature", 0.85)
                ),
    ...
    def _handle_stream_response(self, model_name, messages, parameters):
        ...
        try:
            ...
            for response in responses:
                ...
                # Add reasoning_content (thinking process from cowagent.models like qwen3.5)
                ...
                if tool_calls:
                    openai_chunk["choices"][0]["delta"]["tool_calls"] = (
                        self._convert_tool_calls_to_openai_format(tool_calls)
                    )
