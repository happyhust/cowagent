...
class XunFeiBot(Bot):
    ...
    def reply(self, query, context: Context = None) -> Reply:
        if context.type == ContextType.TEXT:
            ...
            threading.Thread(
                target=self.create_web_socket, args=(session.messages, request_id)
            ).start()
            depth = 0
            time.sleep(0.1)
            t1 = time.time()
            ...
            self.sessions.session_reply(
                reply_map[request_id], session_id, usage.get("total_tokens")
            )
    ...
    def gen_request_id(self, session_id: str):
        return (
            session_id + "_" + str(int(time.time())) + "" + str(random.randint(0, 100))
        )
