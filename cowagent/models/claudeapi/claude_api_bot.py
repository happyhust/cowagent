...
class ClaudeAPIBot(Bot, OpenAIImage):
    ...
    def reply_text(self, session: BaiduWenxinSession, retry_count=0, tools=None):
        try:
            ...
            response = requests.post(
                f"{self.api_base}/messages", headers=headers, json=data, proxies=proxies
            )
    ...
    def _get_max_tokens(self, model: str) -> int:
        ...
        if model and (model.startswith("claude-3-5") or model.startswith("claude-3-7")):
            ...
            model.startswith("claude-sonnet-4") or model.startswith("claude-opus-4")
        ):
            return 64000
        return 8192
                        ...
                        try:
                            ...
                            if event_type == "content_block_start":
                                ...
                                if "stop_reason" in delta:
                                    ...
                                    logger.info(
                                        f"[Claude] Stream stop_reason: {stop_reason}"
                                    )
                                ...
                                logger.debug(
                                    f"[Claude] Stream completed with stop_reason: {stop_reason}"
                                )
