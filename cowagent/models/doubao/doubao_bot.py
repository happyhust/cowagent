...
class DoubaoBot(Bot):
    ...
    def reply_text(
        self, session: DoubaoSession, args=None, retry_count: int = 0
    ) -> dict:
        """
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                logger.error(
                    f"[DOUBAO] chat failed, status_code={res.status_code}, "
            ...
            logger.error(
                f"[DOUBAO] reply_text error | model={self.args.get('model')} | "
    ...
    def call_with_tools(self, messages, tools=None, stream: bool = False, **kwargs):
        ...
        try:
            ...
            error_msg = str(e)
            model_name = kwargs.get("model", self.args.get("model", "doubao-pro"))
            msg_count = len(messages) if messages else 0
            tool_count = len(tools) if tools else 0
            logger.error(
                f"[DOUBAO] call_with_tools error | model={model_name} | "
    ...
    def _handle_stream_response(self, request_body: dict):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[DOUBAO] API error: status={response.status_code}, msg={error_msg}"
                )
                yield {
                    "error": True,
                    "message": error_msg,
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    logger.warning(
                        f"[DOUBAO] JSON decode error: {e}, data: {data_str[:200]}"
                    )
                    continue
