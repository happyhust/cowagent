...
class BaiduWenxinBot(Bot):
    def __init__(self):
        ...
        if self.prompt_enabled:
            ...
            if self.prompt == "":
                logger.warn(
                    "[BAIDU] Although you enabled model prompt, character_desc is not specified."
                )
        ...
        if context and context.type:
            if context.type == ContextType.TEXT:
                ...
                if query == "#清除记忆":
                    ...
                    logger.debug(
                        "[BAIDU] new_query={}, session_id={}, reply_cont={}, completion_tokens={}".format(
                            session.messages,
                            session_id,
                            reply_content,
                            completion_tokens,
    ...
    def reply_text(self, session: BaiduWenxinSession, retry_count=0):
        try:
            ...
            payload = (
                {"messages": session.messages, "system": self.prompt}
                if self.prompt_enabled
                else {"messages": session.messages}
            )
